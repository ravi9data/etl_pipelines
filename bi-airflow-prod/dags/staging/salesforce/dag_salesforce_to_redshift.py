...
from dags.staging.salesforce.config import sf_obj_sync_config
...
DAG_ID = 'salesforce_to_redshift_refresh'
...
with DAG(DAG_ID,
         ...
         description='DAG to sync data from Salesforce objects history tables',
         schedule_interval=get_schedule('@hourly'),
         max_active_runs=1,
         catchup=False,
         tags=[DAG_ID, 'salesforce'],
         ) as dag:
    s3_bucket = '{{ var.value.s3_bucket_curated_bi }}'
    config = sf_obj_sync_config

    upsert = PostgresOperator(
        task_id='run_upsert_sql',
        sql='./sf_stg_upsert.sql',
    ...
    for rec in config:
        ...
        with TaskGroup(f'{sf_object}_refresh', prefix_group_id=False) as sf_obj_refresh:
            s3_key = 'salesforce/{}/{}.json'.format('{{ execution_date }}', sf_object)
            ...
            salesforce_to_s3 = SalesforceToS3Operator(
                ...
                executor_config=rec.get('executor_config', EXECUTOR_CONFIG),
                dag=dag)
