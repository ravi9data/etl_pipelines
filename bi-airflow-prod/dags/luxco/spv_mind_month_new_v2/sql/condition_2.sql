--Union source - Condition Two

DROP TABLE IF EXISTS ods_spv_historical.union_sources_2_mid_month;
CREATE TABLE ods_spv_historical.union_sources_2_mid_month as
SELECT
   *
FROM
...
DROP TABLE IF EXISTS ods_spv_historical.spv_used_asset_price_2_mid_month;
create table ods_spv_historical.spv_used_asset_price_2_mid_month as with d as
(
   /*#############################################
...
###################################################*/
   select DISTINCT
      ...
      mm.avg_mm_price
   from
      ods_spv_historical.union_sources_2_mid_month s      ---*************
      left join
         d mm
...
order by
   3 desc ;
------STEP 3---------
drop table if exists ods_spv_historical.spv_used_asset_price_master_2_mid_month;
---*************
create table ods_spv_historical.spv_used_asset_price_master_2_mid_month as ---*************
WITH a AS
(
   SELECT
   ...
   FROM
      ods_spv_historical.spv_used_asset_price_2_mid_month u     ---*************
   WHERE
...
--------STEP 4-----------------
DROP TABLE IF EXISTS ods_spv_historical.price_per_condition_2_mid_month;
---*************
create table ods_spv_historical.price_per_condition_2_mid_month AS---*************
WITH a AS
(
   SELECT DISTINCT
      b.product_sku,
   ...
   FROM
       ods_spv_historical.spv_used_asset_price_master_2_mid_month b      ---*************
   where
