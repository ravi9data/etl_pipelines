 --Union source - Condition Six

DROP TABLE IF EXISTS ods_spv_historical.union_sources_6;
CREATE TABLE ods_spv_historical.union_sources_6 as
SELECT
   *
FROM
...
DROP TABLE IF EXISTS ods_spv_historical.spv_used_asset_price_6;
create table ods_spv_historical.spv_used_asset_price_6 as with d as
(
   /*#############################################
...
###################################################*/
   select DISTINCT
      ...
      mm.avg_mm_price
   from
      ods_spv_historical.union_sources_6 s		---*************
      left join
         d mm
...
drop table if exists ods_spv_historical.spv_used_asset_price_master_6;
---*************
create table ods_spv_historical.spv_used_asset_price_master_6 as ---*************
WITH a AS
(
   SELECT
   ...
   FROM
      ods_spv_historical.spv_used_asset_price_6 u		---*************
   WHERE
...
DROP TABLE IF EXISTS ods_spv_historical.price_per_condition_6;
---*************
create table ods_spv_historical.price_per_condition_6 AS---*************
WITH a AS
(
   SELECT DISTINCT
      b.product_sku,
   ...
   FROM
      ods_spv_historical.spv_used_asset_price_master_6 b		---*************
   where
