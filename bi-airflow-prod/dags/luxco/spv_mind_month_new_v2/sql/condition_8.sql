  --Union source - Condition Eight

DROP TABLE IF EXISTS ods_spv_historical.union_sources_8;
CREATE TABLE ods_spv_historical.union_sources_8 as
SELECT
   *
FROM
...
DROP TABLE IF EXISTS ods_spv_historical.spv_used_asset_price_8;
create table ods_spv_historical.spv_used_asset_price_8 as with d as
(
   /*#############################################
...
###################################################*/
   select DISTINCT
      ...
      mm.avg_mm_price
   from
      ods_spv_historical.union_sources_8 s		---*************
      left join
         d mm
...
drop table if exists ods_spv_historical.spv_used_asset_price_master_8;
---*************
create table ods_spv_historical.spv_used_asset_price_master_8 as ---*************
WITH a AS
(
   SELECT
   ...
   FROM
      ods_spv_historical.spv_used_asset_price_8 u		---*************
   WHERE
...
DROP TABLE IF EXISTS ods_spv_historical.price_per_condition_8;
---*************
create table ods_spv_historical.price_per_condition_8 AS---*************
WITH a AS
(
   SELECT DISTINCT
      b.product_sku,
   ...
   FROM
      ods_spv_historical.spv_used_asset_price_master_8 b		---*************
   where
