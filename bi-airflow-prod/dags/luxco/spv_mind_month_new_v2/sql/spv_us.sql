   ...
   from
      (
         ...
         where
            ...
            and reporting_month::date < '{{ params.mid_day_of_month }}'::date
         union all
         select distinct
         ...
         where
            week_date::date < '{{ params.mid_day_of_month }}'::date
      )
   group by
      1,
      2
...
SELECT
   ...
   CASE
      ...
      ELSE
         NULL::text
   END
) AS max_available_price_date, datediff(MONTH, b.max_available_reporting_month::date, '{{ params.mid_day_of_month }}'::Date) AS months_since_last_price, power(0.97, datediff(MONTH, b.max_available_reporting_month::date, '{{ params.mid_day_of_month }}'::Date)) * avg(
   CASE
      WHEN
         date_trunc('month', a.reporting_month::date) = date_trunc('month', b.max_available_reporting_month::date)
      THEN
...
#####################*/
         ...
         WHEN
            (
(1 - 0.03 * ((date_trunc('month', '{{ params.first_day_of_month }}'::date - 1)::date - ss.purchased_date) / 30)) * COALESCE(ss.initial_price, ss.amount_rrp, e.initial_price)
            )
            <= 0
         THEN
            0
