...
FROM
   ...
   LEFT JOIN
      ...
      AND ah.date = sh.date
WHERE
   ah.date <= '{{ params.mid_day_of_month }}'
GROUP BY
   1 ;
----------------------------------------------------------------------------------------------------------------------------------------
...
WHERE
   TRUE
   AND DATE_TRUNC('MONTH', s.date) = '{{ params.first_day_of_month }}'
...
WHERE
   TRUE
   AND DATE_TRUNC('MONTH', s.date)::DATE = '{{ params.first_day_of_month }}'
   AND a.allocation_status_original NOT IN
   (
      'CANCELLED'
   )
...
(
   ...
   WHERE
      ...
      AND ah.date <= '{{ params.mid_day_of_month }}'
)
SELECT
   *
FROM
...
WHERE
   ch.date = '{{ params.mid_day_of_month }}' ;
-------------------------------------------------------------------------------------------------------------------------------------

DROP TABLE IF EXISTS ods_spv_historical.customer_score;
CREATE TABLE ods_spv_historical.customer_score AS WITH spain_equifax AS
(
   SELECT DISTINCT
      customer_id::INT,
   ...
   WHERE
      ...
      AND updated_at <= '{{ params.mid_day_of_month }}'
)
,
germany_schufa AS
(
...
(
   ...
   WHERE
	  creation_timestamp <= '{{ params.mid_day_of_month }}'
),
kvm AS
(
   SELECT DISTINCT
   ...
   WHERE
	  updated_at <= '{{ params.mid_day_of_month }}'
),
nl_experian AS
(
   SELECT DISTINCT
...
,
   ...
   (
      ...
      WHERE
         reporting_date = DATE('{{ params.first_day_of_month }}') - 1
   )
,
   allocation_pre AS
   (
...
,
      ...
      WHERE TRUE 
        ...
        AND CASE 
         ...
         WHEN capital_source_name NOT IN ('Grover Finance I GmbH','USA_test')
          ...
          THEN TRUE  
         END
        AND reporting_date = '{{ params.mid_day_of_month }}'
    )
   SELECT DISTINCT
      spv.reporting_date,
      spv.asset_id,
 ...
 , '[^[:alnum:][:blank:][:punct:]]', ''), ' ', ' ') AS asset_name, spv.product_sku, spv.warehouse, spv.capital_source_name, acat.category, acat.subcategory, spv.purchased_date, spv.initial_price AS purchase_price,
      ...
      spv.last_allocation_days_in_stock AS days_in_stock, 
...
GRANT
SELECT
   ON ods_spv_historical.luxco_reporting_{{ params.tbl_suffix }} to paolo_giambona;
GRANT
SELECT
