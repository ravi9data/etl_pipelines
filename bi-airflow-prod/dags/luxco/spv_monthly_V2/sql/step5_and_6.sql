...
#####################*/
         ...
         WHEN
            (
(1 - 0.03 * ((date_trunc('month', '{{ params.last_day_of_prev_month }}'::date)::date - ss.purchased_date) / 30)) * COALESCE(f.gross_purchase_price_corrected, ss.initial_price, ss.amount_rrp, e.initial_price)
            )
            <= 0
         THEN
            0
