...
DROP TABLE IF EXISTS ods_spv_historical.union_sources_2;
CREATE TABLE ods_spv_historical.union_sources_2 as
SELECT
   *
FROM
...
DROP TABLE IF EXISTS ods_spv_historical.spv_used_asset_price_2;
create table ods_spv_historical.spv_used_asset_price_2 as with d as
(
   /*#############################################
...
###################################################*/
   select DISTINCT
      ...
      mm.avg_mm_price
   from
      ods_spv_historical.union_sources_2 s		---*************
      left join
         d mm
...
----------------------------------------------------------
----------------------------------------------------------
drop table if exists ods_spv_historical.spv_used_asset_price_master_2;
---*************
create table ods_spv_historical.spv_used_asset_price_master_2 as ---*************
WITH a AS
(
   SELECT
   ...
   FROM
      ods_spv_historical.spv_used_asset_price_2 u		---*************
   WHERE
...
DROP TABLE IF EXISTS ods_spv_historical.price_per_condition_2;
---*************
create table ods_spv_historical.price_per_condition_2 AS---*************
WITH a AS
(
   SELECT DISTINCT
      b.product_sku,
   ...
   FROM
      ods_spv_historical.spv_used_asset_price_master_2 b		---*************
   where
