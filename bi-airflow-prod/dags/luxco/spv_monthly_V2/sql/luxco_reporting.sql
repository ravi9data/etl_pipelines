DROP TABLE IF EXISTS ods_spv_historical.cashflow_due;
CREATE TABLE ods_spv_historical.cashflow_due AS
SELECT DISTINCT
   pa.asset_id,
...
GROUP BY
   1, 2;
-------------------------------------------------------------------------------------------------------------------------------------
DROP TABLE IF EXISTS ods_spv_historical.cashflow;
CREATE TABLE ods_spv_historical.cashflow AS
SELECT DISTINCT
   pa.asset_id,
...
GROUP BY
   1, 2 ;
-------------------------------------------------------------------------------------------------------------------------------------
DROP TABLE IF EXISTS ods_spv_historical.paid_reporting_month;
CREATE TABLE ods_spv_historical.paid_reporting_month AS
SELECT DISTINCT
   spv.asset_id,
   spv.reporting_date::DATE,
   SUM(COALESCE(subscription_revenue_paid, 0)) AS subscription_revenue_paid_reporting_month,
   SUM(COALESCE(customer_bought_paid, 0) + COALESCE(grover_sold_paid, 0)) AS asset_sales_revenue_paid_reporting_month,
...
FROM
   ods_spv_historical.cashflow acc
   INNER JOIN
      ods_production.spv_report_master spv
...
GROUP BY
   ...
   2 ;
-------------------------------------------------------------------------------------------------------------------------------------
DROP TABLE IF EXISTS ods_spv_historical.due_reporting_month;
CREATE TABLE ods_spv_historical.due_reporting_month AS
SELECT DISTINCT
   spv.asset_id,
   spv.reporting_date::DATE,
   ...
   SUM(COALESCE(repair_cost_due, 0)) + SUM(COALESCE(shipment_cost_due, 0)) + SUM(COALESCE(customer_bought_due, 0)) + SUM(COALESCE(grover_sold_due, 0)) + SUM(COALESCE(debt_collection_due, 0)) + SUM(COALESCE(additional_charge_due, 0)) + SUM(COALESCE(asset_payment_chargeback_due, 0)) + SUM(COALESCE(refund_asset_payment_due, 0)) + SUM(COALESCE(refund_other_payment_due, 0)) AS other_revenue_due_reporting_month,
   --total revenue net due
   SUM(COALESCE(repair_cost_due, 0)) + SUM(COALESCE(subscription_revenue_due, 0)) + SUM(COALESCE(shipment_cost_due, 0)) + SUM(COALESCE(customer_bought_due, 0)) + SUM(COALESCE(grover_sold_due, 0)) + SUM(COALESCE(debt_collection_due, 0)) + SUM(COALESCE(additional_charge_due, 0)) + SUM(COALESCE(sub_payment_chargeback_due, 0)) + SUM(COALESCE(asset_payment_chargeback_due, 0)) + SUM(COALESCE(refund_sub_payment_due, 0)) + SUM(COALESCE(refund_asset_payment_due, 0)) + SUM(COALESCE(refund_other_payment_due, 0)) AS total_revenue_due_reporting_month
FROM
   ods_spv_historical.cashflow_due acc
   INNER JOIN
      ods_production.spv_report_master spv
      ON spv.asset_id = acc.asset_Id
...
GROUP BY
   ...
   2 ;
-------------------------------------------------------------------------------------------------------------------------------------
DROP TABLE IF EXISTS ods_spv_historical.lifetime;
CREATE TABLE ods_spv_historical.lifetime AS
SELECT DISTINCT
   spv.asset_id,
   spv.reporting_date::DATE,
   SUM(COALESCE(subscription_revenue_paid, 0)) AS subscription_revenue_paid_lifetime,
   SUM(COALESCE(customer_bought_paid, 0) + COALESCE(grover_sold_paid, 0)) AS asset_sales_revenue_paid_lifetime,
...
GROUP BY
   ...
   2 ;
-------------------------------------------------------------------------------------------------------------------------------------
DROP TABLE IF EXISTS ods_spv_historical.due_lifetime;
CREATE TABLE ods_spv_historical.due_lifetime AS
SELECT DISTINCT
   spv.asset_id,
   spv.reporting_date::DATE,
   --subscription revenue net due
   ...
   --other revenue net due
   SUM(COALESCE(repair_cost_due, 0)) + SUM(COALESCE(shipment_cost_due, 0)) + SUM(COALESCE(customer_bought_due, 0)) + SUM(COALESCE(grover_sold_due, 0)) + SUM(COALESCE(debt_collection_due, 0)) + SUM(COALESCE(additional_charge_due, 0)) + SUM(COALESCE(asset_payment_chargeback_due, 0)) + SUM(COALESCE(refund_asset_payment_due, 0)) + SUM(COALESCE(refund_other_payment_due, 0)) AS other_revenue_due_lifetime,
   --total revenue net due
   SUM(COALESCE(repair_cost_due, 0)) + SUM(COALESCE(subscription_revenue_due, 0)) + SUM(COALESCE(shipment_cost_due, 0)) + SUM(COALESCE(customer_bought_due, 0)) + SUM(COALESCE(grover_sold_due, 0)) + SUM(COALESCE(debt_collection_due, 0)) + SUM(COALESCE(additional_charge_due, 0)) + SUM(COALESCE(sub_payment_chargeback_due, 0)) + SUM(COALESCE(asset_payment_chargeback_due, 0)) + SUM(COALESCE(refund_sub_payment_due, 0)) + SUM(COALESCE(refund_asset_payment_due, 0)) + SUM(COALESCE(refund_other_payment_due, 0)) AS total_revenue_due_lifetime
FROM
   ods_spv_historical.cashflow_due acc
   INNER JOIN
      ods_production.spv_report_master spv
...
GROUP BY
   ...
   2 ;
-------------------------------------------------------------------------------------------------------------------------------------
DROP TABLE IF EXISTS ods_spv_historical.subs_per_asset;
CREATE TABLE ods_spv_historical.subs_per_asset AS
SELECT
   ah.asset_id,
...
FROM
   ...
   LEFT JOIN
      ...
      AND ah.date = sh.date
WHERE
   ah.date <= '{{ params.last_day_of_prev_month }}'
GROUP BY
   1 ;
----------------------------------------------------------------------------------------------------------------------------------------
DROP TABLE IF EXISTS ods_spv_historical.sub_recon;
CREATE TABLE ods_spv_historical.sub_recon AS
SELECT
...
WHERE
   TRUE
   AND DATE_TRUNC('MONTH', s.date) = '{{ params.date_for_depreciation }}'
...
GROUP BY
   1, 2 ;
-------------------------------------------------------------------------------------------------------------------------------------
DROP TABLE IF EXISTS ods_spv_historical.active_value;
CREATE TABLE ods_spv_historical.active_value AS
SELECT DISTINCT
...
WHERE
   TRUE
   AND DATE_TRUNC('MONTH', s.date)::DATE = '{{ params.date_for_depreciation }}'
   AND a.allocation_status_original NOT IN
   (
      'CANCELLED'
   )
...
GROUP BY
   1, 2, 3, 4 ;
-------------------------------------------------------------------------------------------------------------------------------------
DROP TABLE IF EXISTS ods_spv_historical.cancelled_value;
CREATE TABLE ods_spv_historical.cancelled_value AS
SELECT DISTINCT
...
-------------------------------------------------------------------------------------------------------------------------------------
DROP TABLE IF EXISTS ods_spv_historical.asset_historical;
CREATE TABLE ods_spv_historical.asset_historical AS
SELECT
   *
FROM
   master.asset_historical
WHERE
   date = '{{ params.last_day_of_prev_month }}' ;
-------------------------------------------------------------------------------------------------------------------------------------
DROP TABLE IF EXISTS ods_spv_historical.subscription_historical;
CREATE TABLE ods_spv_historical.subscription_historical AS
SELECT
   *
FROM
...
WHERE
   date = '{{ params.last_day_of_prev_month }}' ;
-------------------------------------------------------------------------------------------------------------------------------------
DROP TABLE IF EXISTS ods_spv_historical.order_historical;
CREATE TABLE ods_spv_historical.order_historical AS
SELECT
   *
FROM
...
WHERE
   date = '{{ params.last_day_of_prev_month }}' ;
-------------------------------------------------------------------------------------------------------------------------------------
DROP TABLE IF EXISTS ods_spv_historical.spv_report_master_{{ params.tbl_suffix }};
CREATE TABLE ods_spv_historical.spv_report_master_{{ params.tbl_suffix }} AS
SELECT
   *
FROM
   ods_production.spv_report_master_{{ params.tbl_suffix }} ;
-------------------------------------------------------------------------------------------------------------------------------------
DROP TABLE IF EXISTS ods_spv_historical.supplier;
CREATE TABLE ods_spv_historical.supplier AS
SELECT
   *
FROM
   ods_production.supplier ;
-------------------------------------------------------------------------------------------------------------------------------------
DROP TABLE IF EXISTS ods_spv_historical.asset_category;
CREATE TABLE ods_spv_historical.asset_category AS WITH asset_category_base AS
(
   SELECT
      asset_id,
   ...
   WHERE
      ...
      AND ah.date <= '{{ params.last_day_of_prev_month }}'
)
SELECT
   *
FROM
...
WHERE
   cat_rank = 1 ;
-------------------------------------------------------------------------------------------------------------------------------------
DROP TABLE IF EXISTS ods_spv_historical.cust;
CREATE TABLE ods_spv_historical.cust AS
SELECT
   ch.customer_id,
   ch.customer_type,
...
WHERE
   ch.date = '{{ params.last_day_of_prev_month }}' ;
-------------------------------------------------------------------------------------------------------------------------------------

DROP TABLE IF EXISTS ods_spv_historical.customer_score;
CREATE TABLE ods_spv_historical.customer_score AS 
WITH spain_equifax AS
(
   SELECT DISTINCT
      customer_id::INT,
   ...
   WHERE
      ...
      AND updated_at <= '{{ params.last_day_of_prev_month }}'
)
,
germany_schufa AS
(
...
(
   SELECT DISTINCT
      customer_id::INT,
      'de_crif' AS score_provider,
      LAST_VALUE(score_value::DECIMAL(38, 2)) OVER (PARTITION BY customer_id
   ORDER BY
      updated_at ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS de_crif_score_value
   FROM
...
(
   SELECT DISTINCT
      customer_id::INT,
      'at_crif' AS score_provider,
      LAST_VALUE(score_value::DECIMAL(38, 2)) OVER (PARTITION BY customer_id
   ORDER BY
      updated_at ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS at_crif_score_value
   FROM
      s3_spectrum_rds_dwh_order_approval.crifburgel_data s
   WHERE TRUE
     AND country_code = 'AT' 
...
(
   ...
   WHERE
	  creation_timestamp <= '{{ params.last_day_of_prev_month }}'
),
kvm AS
(
   SELECT DISTINCT
   ...
   WHERE
	  updated_at <= '{{ params.last_day_of_prev_month }}'
),
nl_experian AS
(
   SELECT DISTINCT
...
SELECT DISTINCT
   ...
   gf.de_crif_score_value,
   af.at_crif_score_value,
   burgel.burgel_score,
   kvm.kvm_score,
...
FROM
   ...
   LEFT JOIN
      germany_crif gf
      ON cust.customer_id = gf.customer_id
   LEFT JOIN
      austria_crif af
      ON cust.customer_id = af.customer_id      
   LEFT JOIN
      burgel
...
-------------------------------------------------------------------------------------------------------------------------------------
DROP TABLE IF EXISTS ods_spv_historical.asset_cohort_month;
CREATE TABLE ods_spv_historical.asset_cohort_month AS
SELECT DISTINCT
   asset_id,
...
-------------------------------------LUXCO REPORTING----------------------------------------------------------------------------------------------
-------------------------------------LUXCO REPORTING----------------------------------------------------------------------------------------------
DROP TABLE IF EXISTS ods_spv_historical.luxco_reporting_{{ params.tbl_suffix }};
CREATE TABLE ods_spv_historical.luxco_reporting_{{ params.tbl_suffix }} AS
(
   ...
   (
      ...
      FROM
         ods_spv_historical.active_value
      GROUP BY
         1, 2
   )
, cancelled_value_final AS
   (
      ...
      FROM
         ods_spv_historical.cancelled_value
      GROUP BY
         1,
         2
   )
...
,
   ...
   (
      ...
      WHERE
         reporting_date = DATE('{{ params.date_for_depreciation }}') - 1
   )
,
   allocation_pre AS
   (
  ...
  dpd_correction AS 
     (
      SELECT 
       ...
       ,dpd
       ,dpd_new
      FROM dm_finance.sub_dpd_corrected_final
      WHERE TRUE 
        AND COALESCE(dpd_new ,9999) <= 360
        AND CASE 
         WHEN capital_source_name = 'Grover Finance I GmbH' 
           AND COALESCE(dpd ,99999) > 360
           AND COALESCE(dpd_new ,99999) <= 360  
          THEN TRUE 
         WHEN capital_source_name = 'USA_test'   
           AND COALESCE(dpd ,99999) > 180 
           AND COALESCE(dpd_new ,99999) <= 180
          THEN TRUE          
         WHEN capital_source_name = 'USA_test'   
           AND COALESCE(dpd ,99999) > 270 
           AND COALESCE(dpd_new ,99999) <= 270
          THEN TRUE       
         WHEN capital_source_name = 'USA_test' 
           AND COALESCE(dpd ,99999) > 360
           AND COALESCE(dpd_new ,99999) <= 360      
          THEN TRUE  
         WHEN capital_source_name NOT IN ('Grover Finance I GmbH','USA_test')
           AND COALESCE(dpd ,99999) > 90 
           AND COALESCE(dpd_new ,99999) <= 90
          THEN TRUE        
         WHEN capital_source_name NOT IN ('Grover Finance I GmbH','USA_test')
           AND COALESCE(dpd ,99999) > 180 
           AND COALESCE(dpd_new ,99999) <= 180
         ...
         WHEN capital_source_name NOT IN ('Grover Finance I GmbH','USA_test')
          ...
          THEN TRUE  
         END
        AND reporting_date = '{{ params.last_day_of_prev_month }}'
    )
   SELECT DISTINCT
      spv.reporting_date,
      spv.asset_id,
 ...
 , '[^[:alnum:][:blank:][:punct:]]', ''), ' ', ' ') AS asset_name, spv.product_sku, spv.warehouse, spv.capital_source_name, acat.category, acat.subcategory, spv.purchased_date, spv.initial_price AS purchase_price,
      ...
      CASE
         ...
         ELSE
            0
      END
      AS asset_added_to_portfolio_m_and_g, spv.last_allocation_days_in_stock AS days_in_stock
      ,COALESCE(dc.dpd_new, h.last_allocation_dpd) AS last_allocation_dpd,
      CASE
         WHEN
         ...
         THEN
            'IN STOCK'
         WHEN
            spv.asset_status_original LIKE 'WRITTEN OFF%' AND COALESCE(dc.dpd_new, 9999) > 360
         THEN
            'WRITTEN OFF'
         WHEN
            COALESCE(dc.dpd_new, h.last_allocation_dpd) BETWEEN 1 AND 30
         THEN
            'OD 1-30'   
         WHEN
            COALESCE(dc.dpd_new, h.last_allocation_dpd) BETWEEN 31 AND 60
         THEN
            'OD 31-60'   
         WHEN
            COALESCE(dc.dpd_new, h.last_allocation_dpd) BETWEEN 61 AND 90
         THEN
            'OD 61-90'   
         WHEN
            COALESCE(dc.dpd_new, h.last_allocation_dpd) BETWEEN 91 AND 120
         THEN
            'OD 91-120'   
         WHEN
            COALESCE(dc.dpd_new, h.last_allocation_dpd) BETWEEN 121 AND 150
         THEN
            'OD 121-150'   
         WHEN
            COALESCE(dc.dpd_new, h.last_allocation_dpd) BETWEEN 151 AND 180
         THEN
            'OD 151-180'
         WHEN
            COALESCE(dc.dpd_new, h.last_allocation_dpd) BETWEEN 181 AND 270
         THEN
            'OD 181-270'
         WHEN
            COALESCE(dc.dpd_new, h.last_allocation_dpd) BETWEEN 271 AND 360
         THEN
            'OD 271-360'
         WHEN
            COALESCE(dc.dpd_new, h.last_allocation_dpd) > 360
         THEN
            'OD 360+'
         ELSE
            h.dpd_bucket
      ...
      ,CASE
      	WHEN spv.asset_status_original LIKE '%WRITTEN OFF%'
      	  AND dc.dpd_new IS NOT NULL 
      	 THEN 'ON LOAN'
      	ELSE spv.asset_status_original 
      END AS asset_status_original, ah.allocation_status_original, m_since_last_valuation_price, valuation_method, spv.currency, h.asset_value_linear_depr AS discounted_purchase_price
      ,CASE 
      	WHEN dc.dpd_new IS NOT NULL AND COALESCE(spv.final_price  ,0) > COALESCE(spv.final_price_without_written_off ,0) 
      	 THEN spv.final_price
      	WHEN dc.dpd_new IS NOT NULL AND COALESCE(spv.final_price  ,0) <= COALESCE(spv.final_price_without_written_off ,0)           
      	 THEN spv.final_price_without_written_off
         ELSE spv.final_price
      END AS original_valuation    
      ,CASE
         WHEN
         ...
         THEN
            CASE
               WHEN
                  COALESCE(dc.dpd_new, last_allocation_dpd) < 361
               THEN
                  0
               WHEN
                  COALESCE(dc.dpd_new, last_allocation_dpd) >= 361
            ...
            THEN
               CASE
                  WHEN
                     COALESCE(dc.dpd_new, last_allocation_dpd) BETWEEN 181 AND 270
                  THEN
                     0.25
                  WHEN
                     COALESCE(dc.dpd_new, last_allocation_dpd) BETWEEN 271 AND 360
                  THEN
                     0.5
                  WHEN
                     COALESCE(dc.dpd_new, last_allocation_dpd) >= 361
            ...
            THEN
               CASE
                  WHEN
                     COALESCE(dc.dpd_new, last_allocation_dpd) BETWEEN 91 AND 180
                  THEN
                     0.25
                  WHEN
                     COALESCE(dc.dpd_new, last_allocation_dpd) BETWEEN 181 AND 270
 ...
 , '[^[:alnum:][:blank:][:punct:]]', ''), ' ', ' ')
	 ...
	 THEN
      ...
      CASE
         ...
         THEN
            cust_sco.at_crif_score_value::TEXT
         WHEN
         ...
         THEN
            COALESCE(cust_sco.de_schufa_score_rating, cust_sco.de_crif_score_value::TEXT, cust_sco.burgel_score::TEXT)::TEXT
      END
      AS customer_score,
      CASE
         WHEN
         ...
         WHEN
            COALESCE(s.country_name, cust.shipping_country) = 'Austria'
            AND cust_sco.at_crif_score_value IS NOT NULL
         THEN
            'at_crif'
         WHEN
            COALESCE(s.country_name, cust.shipping_country) = 'Spain'
         ...
         WHEN
            COALESCE(s.country_name, cust.shipping_country) = 'Germany'
            AND cust_sco.de_crif_score_value IS NOT NULL
         THEN
            'de_crif'
         WHEN
            COALESCE(s.country_name, cust.shipping_country) = 'Germany'
      ...
      CASE
         ...
         WHEN
            ...
            AND cust_sco.at_crif_score_value < 420
         THEN
            TRUE
         WHEN
            credit_buro = 'de_crif'
            AND COALESCE(s.country_name, cust.shipping_country) = 'Germany'
            AND cust_sco.de_crif_score_value > 4
         THEN
            TRUE
         WHEN
	...
	THEN 
     ...
     FROM
      ods_spv_historical.spv_report_master_{{ params.tbl_suffix }} spv
      LEFT JOIN
         ods_spv_historical.asset_historical AS h
      ...
      LEFT JOIN
         ods_spv_historical.asset_category acat
         ON spv.asset_id = acat.asset_id
      LEFT JOIN
         ods_spv_historical.asset_cohort_month chm
         ON spv.asset_id = chm.asset_id
      LEFT JOIN
         ods_spv_historical.customer_score cust_sco
      ...
      LEFT JOIN dpd_correction dc
        ON ah.subscription_id = dc.subscription_id
   WHERE
      spv.reporting_date = '{{ params.last_day_of_prev_month }}' ) ; 
-----------------------------------------LUCXO REPORTING END--------------------------------------------------------------------------------------------
-----------------------------------------LUCXO REPORTING END--------------------------------------------------------------------------------------------

--GRANT ALL ON ALL tables IN SCHEMA ods_spv_historical TO erkin_unler, radwa_hosny, 
--nasim_pezeshki, saad_amir, ceyda_peker, pawan_pai;

--GRANT SELECT ON ALL tables IN SCHEMA ods_spv_historical TO elene_tsintsadze,
--paolo_giambona, basri_oz;

GRANT
SELECT
   ON ods_spv_historical.luxco_reporting_{{ params.tbl_suffix }} to elene_tsintsadze;
   
GRANT
SELECT
   ON ods_spv_historical.luxco_reporting_{{ params.tbl_suffix }} TO accounting_redash;
   
GRANT
SELECT
   ON ods_spv_historical.luxco_reporting_{{ params.tbl_suffix }} TO fabio_zaim;