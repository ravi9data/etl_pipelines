...
left join dm_finance.luxco_report_master lrm
    on lrm.serial_number = skf.serialnumber
    and cast(lrm.reporting_date as date) < cast(skf.date_of_processing as date)
    ) a
where rn=1
),
...
select * from(
    ...
    row_number() over(partition by lrm.serial_number order by lrm.reporting_date desc) as rn
    from dm_recommerce.spl_kd_final skf
    left join dm_finance.luxco_report_master lrm  on lrm.serial_number = skf.serialnumber
        and cast(lrm.reporting_date as date) >= cast(skf.date_of_processing as date)
)
where rn=1
),
...
select * from (
    ...
    left join master.allocation_historical ah
        on ah.asset_id = a.asset_id
        and cast(ah."date" as date) > cast(skf.date_of_processing as date)
...
grant select on table dm_recommerce.recirculation_analytics to GROUP bi;
