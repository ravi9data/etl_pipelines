...
from business_logic.intercom.data_transformations import (
    get_init_starting_after, get_page_count, get_starting_after,
    transform_dataframe, transform_response)
...
def ingest_conversation_id(**context):
    ...
    for page_range in range(total_pages):
        if page_range == 0:
            ...
            for i in range(per):
                conv_id = parsed_json['conversations'][i]['id']
                created_at = parsed_json['conversations'][i]['created_at']
                updated_at = parsed_json['conversations'][i]['updated_at']
                conver_id.append(conv_id)
                ct_at.append(convert_unix_ts(created_at))
                up_at.append(convert_unix_ts(updated_at))
                extracted_at.append(datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
                year.append(datetime.now().strftime("%Y"))
                month.append(datetime.now().strftime("%m"))
                day.append(datetime.now().strftime("%d"))
            starting_after = get_init_starting_after(headers=headers, params=params)
            params = (
                ('order', 'asc'),
                ('sort', 'created_at'),
                ('per_page', 150),
            ...
            response = requests.get(
                url='https://api.intercom.io/conversations?&page={}&starting_after={}',
                headers=headers,
                params=params)
            starting_next = get_starting_after(headers=headers, params=params)
            params = (
                    ('order', 'asc'),
                    ('sort', 'created_at'),
                    ('per_page', 150),
                    ('starting_after', starting_next))
            data = response.text
            parsed_json = json.loads(data)
            for i in range(per):
                conv_id = parsed_json['conversations'][i]['id']
                ...
                month.append(datetime.now().strftime("%m"))
                day.append(datetime.now().strftime("%d"))
    finaldata = pd.DataFrame({'id': conver_id,
                              'created_at': ct_at,
                              'updated_at': up_at,
