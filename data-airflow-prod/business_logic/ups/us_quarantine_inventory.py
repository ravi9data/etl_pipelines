...
from business_logic.ups.config import use_cols
...
def extract_email_attachment_to_s3(dag_config, **context):
    """
    The target data is inside the email excel attachment, which is extracted,
    and loaded to a Pandas Dataframe. The data is written as CSV format to s3.
    """
    ...
    task_instance.xcom_push(key="reporting_date", value=email_date.strftime('%Y-%m-%d'))

    inventory_data = excel_attachment_to_dataframe(dag_config=dag_config,
                                                   username=username,
                                                   password=password,
    ...
    logger.info(f'Number of records retrieved: {inventory_data.shape[0]}')
    logger.info('Transforming dataframe for writing to S3')
    inventory_data.columns = inventory_data.columns.str.replace(' ', '_')
    inventory_data, batch_id = transform_dataframe(input_df=inventory_data,
                                                   partition_cols=['year', 'month', 'day'])

    logger.info('Writing dataframe to S3 in csv format')
    s3_writer = to_csv(df=inventory_data,
