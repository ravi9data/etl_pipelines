...
from dags.rds_datawarehouse.salesforce_events.config.hourly_load import config
from plugins.dag_utils import get_schedule, on_failure_callback
from plugins.operators.s3.db_to_s3_operator import DbToS3Operator

DAG_ID = 'salesforce_events_hourly_load'
...
dag = DAG(
    ...
    description='Incremental load salesforce_events tables from RDS dwh replica to data lake',
    schedule_interval=get_schedule('5 9,21 * * *'),
    catchup=False,
    max_active_runs=1,
    tags=[DAG_ID, 'salesforce_events', 'hourly', 'rds', 'postgres', 'datawarehouse']
)
...
for conf in config:
    data_load = DbToS3Operator(
        ...
        target_glue_database='{{ var.value.salesforce_events_target_glue_database }}',
