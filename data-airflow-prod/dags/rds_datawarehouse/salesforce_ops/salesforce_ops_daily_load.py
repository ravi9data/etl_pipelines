...
from dags.rds_datawarehouse.salesforce_ops.config.hourly_load import config
from plugins.dag_utils import get_schedule, on_failure_callback
from plugins.operators.s3.db_to_s3_operator import DbToS3Operator

DAG_ID = 'salesforce_ops_daily_load'
...
default_args = {
    ...
    'start_date': datetime.datetime(2022, 1, 27),
    'retries': 3,
...
dag = DAG(
    ...
    description='Incremental load salesforce_ops data from RDS dwh replica to data lake',
    schedule_interval=get_schedule('0 7,19 * * *'),
    catchup=False,
    max_active_runs=1,
    tags=[DAG_ID, 'saleforce_ops', 'daily', 'rds', 'postgres', 'datawarehouse']
)
...
for conf in config:
    data_load = DbToS3Operator(
        ...
        target_glue_database='{{ var.value.salesforce_ops_target_glue_database }}',
