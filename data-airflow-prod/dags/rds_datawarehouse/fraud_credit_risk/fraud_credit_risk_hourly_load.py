...
from dags.rds_datawarehouse.fraud_credit_risk.config.hourly_load import config
from plugins.dag_utils import get_schedule, on_failure_callback
from plugins.operators.s3.db_to_s3_operator import DbToS3Operator

DAG_ID = 'fraud_and_credit_risk_hourly_load'
...
dag = DAG(
    ...
    description='A DAG to load Fraud and Credit Risk data from RDS datawarehouse to the data lake',
    schedule_interval=get_schedule('35 10,21 * * *'),
    catchup=False,
    max_active_runs=1,
    tags=[DAG_ID, 'fraud_and_credit_risk', 'hourly', 'rds', 'postgres', 'datawarehouse']
)
...
for conf in config:
    data_load = DbToS3Operator(
        ...
        target_glue_database='{{ var.value.fraud_credit_and_risk_target_glue_database }}',
