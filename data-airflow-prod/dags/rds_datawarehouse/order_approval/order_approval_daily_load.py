...
from dags.rds_datawarehouse.order_approval.config.daily_load import config
from plugins.dag_utils import get_schedule, on_failure_callback
from plugins.operators.s3.db_to_s3_operator import DbToS3Operator

DAG_ID = 'order_approval_daily_load'
...
default_args = {
    ...
    'start_date': datetime.datetime(2022, 7, 3),
    'retries': 3,
...
dag = DAG(
    ...
    description='Incremental load order_approval data from RDS dwh replica to data lake daily',
    schedule_interval=get_schedule('5 0 * * *'),
    catchup=False,
    max_active_runs=1,
    tags=[DAG_ID, 'order_approval', 'daily', 'rds', 'postgres', 'datawarehouse']
)
...
for conf in config:
    data_load = DbToS3Operator(
        ...
        target_glue_database='{{ var.value.order_approval_target_glue_database }}',
