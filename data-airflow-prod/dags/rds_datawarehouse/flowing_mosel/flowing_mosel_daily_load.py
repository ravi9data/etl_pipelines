...
from dags.rds_datawarehouse.flowing_mosel.config.daily_load import config
from plugins.dag_utils import get_schedule, on_failure_callback
from plugins.operators.s3.db_to_s3_operator import DbToS3Operator

DAG_ID = 'flowing_mosel_daily_load'
...
dag = DAG(
    ...
    description='full load flowing_mosel data from RDS dwh replica to data lake',
    schedule_interval=get_schedule('0 23 * * *'),
    catchup=False,
    max_active_runs=1,
    tags=[DAG_ID, 'flowing_mosel', 'daily', 'rds', 'postgres', 'datawarehouse']
)
...
for conf in config:
    data_load = DbToS3Operator(
        ...
        target_glue_database='{{ var.value.flowing_mosel_target_glue_database }}',
