...
dag = DAG(
    ...
    catchup=False,
    max_active_runs=1,
    tags=[PARENT_FOLDER, 'mozenda', 'amazon', 'prices']
)

get_latest_data_available_for_amazon_task = PythonOperator(
    dag=dag,
    task_id='get_latest_data_available_for_amazon',
    python_callable=get_latest_data_available_for_amazon_eu
)
...
copy_data_task = RedshiftCopyOperator(
    ...
    columns=[
        ...
        "price",
        "crawled_at"],
    s3_location="{{ ti.xcom_pull(task_ids='get_latest_data_available_for_amazon') }}",
    delimiter=','
)
