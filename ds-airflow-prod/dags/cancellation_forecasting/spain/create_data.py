...
import dags.cancellation_forecasting.spain.constants as const
...
def create_dataframe(
    ...
    prophet_list = [s for s in subcategory_list if s not in const.RF_SUBCATEGORIES]
    prophet_list.extend(['Desktop Computers', 'DJ Equipment', 'Laptops'])
    ...
    ts_forecast = predict_ts_target(cohort_data, prophet_list, train_test_cutoff_date)
    ...
    logging.info('Fill variables with 0')
    ...
    logging.info('Project future acquisitions targets for the subcategories '
                 'that use an RF model that doesnt support null values')
