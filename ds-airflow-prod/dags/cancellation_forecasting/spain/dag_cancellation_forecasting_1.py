...
from dags.cancellation_forecasting.spain.cancellation_forecast_process import \
    run_cancellation_forecasts
from plugins.dag_utils import get_schedule, on_failure_callback

DAG_ID = "sub_cancellation_forecasting_spain_1"
...
def cancellation_forecasting_spain_1():
    ...
    logging.info("loading subcategory_list_ES pkl file to dataframe")
    subcategory_data = pd.read_pickle(os.path.join(local_dir, "subcategory_list_ES.pkl"))

    # Table with the models' information. The algorithm, the features, the data, the parameters:
    logging.info("loading final_models_ES_20221018 pkl file to dataframe")
    final_models_table = pd.read_pickle(os.path.join(local_dir,
                                                     "final_models_ES_20221018.pkl"))
...
dag = DAG(
    ...
    description="Run Subscription Cancellation forecasting model for Spain",
    start_date=datetime(2023, 8, 1),
    schedule_interval=get_schedule("50 6 1 * *"),
    max_active_runs=1,
    catchup=False,
    tags=["Automation", "DS"]
)
...
cancellation_forecasting_spain_1 = PythonOperator(
    dag=dag,
    task_id="cancellation_forecasting_spain_1",
    python_callable=cancellation_forecasting_spain_1,
    executor_config={
        "KubernetesExecutor": {
...
t0 >> cancellation_forecasting_spain_1
