...
from dags.cancellation_forecasting.spain import create_data
...
def run_cancellation_forecasts(
    ...
    # The first 2 rows in final_models_table are Prophet and moving average models
    for index, row in final_models_table.iloc[2:, :].iterrows():
    ...
    logging.info('Compute Moving average forecasts')
    MA_subcategories = final_models_table['subcategory_name'].iloc[1]
    MA_final_results = pd.DataFrame()
    for s in MA_subcategories:
        subcategory_forecasts = pd.concat([
            cohort_data[cohort_data.subcategory_name.isin([s])].groupby(['cancellation_date'])[
                ['y']].sum(
            ),
            future_data[future_data.subcategory_name.isin([s])].groupby(['cancellation_date'])[
                ['yhat']].min(
            )
        ]).bfill(axis=1)[['y']].rolling(3).mean().round().loc[train_test_cutoff_date:]
        subcategory_forecasts['subcategory_name'] = s

        MA_final_results = pd.concat([MA_final_results, subcategory_forecasts])

    MA_final_results = MA_final_results.reset_index()
    MA_final_results = MA_final_results.merge(
        avg_asv.groupby(['subcategory_name'])[['subscription_value']].last(), on='subcategory_name',
        how='left')
    MA_final_results['cancelled_asv'] = MA_final_results['y'] * MA_final_results[
        'subscription_value']
    MA_final_results = MA_final_results.rename(
        columns={'y': 'cancelled_subscriptions', 'subcategory_name': 'subcategory'})
    ...
    final_results[['cancelled_asv', 'cancelled_subscriptions']] = final_results[
        ['cancelled_asv', 'cancelled_subscriptions']].round()
    final_results['country'] = 'Spain'
