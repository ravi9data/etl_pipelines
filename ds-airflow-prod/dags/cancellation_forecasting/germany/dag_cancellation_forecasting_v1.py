...
DAG_ID = "sub_cancellation_forecasting_germany_1"
...
def cancellation_forecasting_germany_1():
    ...
    logging.info("loading final_subcategory_models pkl file to dataframe")
    final_models_table = pd.read_pickle(
        os.path.join(local_dir, "final_subcategory_models_20230725_v3.pkl")
    )
...
dag = DAG(
    ...
    schedule_interval=get_schedule("30 6 1 * *"),
    max_active_runs=1,
    catchup=False,
    tags=["Automation", "DS"],
)
...
cancellation_forecasting_germany_1 = PythonOperator(
    dag=dag,
    task_id="cancellation_forecasting_germany_1",
    python_callable=cancellation_forecasting_germany_1,
    executor_config={
        "KubernetesExecutor": {
            "request_memory": "3G",
            "request_cpu": "1500m",
...
t0 >> cancellation_forecasting_germany_1
