...
from dags.cancellation_forecasting.germany_retail.cancellation_forecast_process import \
    run_cancellation_forecasts
from plugins.dag_utils import get_schedule, on_failure_callback

DAG_ID = "sub_cancellation_forecasting_germany_retail_1"
...
def cancellation_forecasting_germany_retail_1():
    ...
    logging.info("loading subcategory_list pkl file to dataframe")
    subcategory_data = pd.read_pickle(os.path.join(local_dir, "subcategory_list_DE_retail.pkl"))

    # Table with the models' information. The algorithm, the features, the data, the parameters:
    logging.info("loading final_models_DE_retail_20221027 pkl file to dataframe")
    final_models_table = pd.read_pickle(os.path.join(local_dir,
                                                     "final_models_DE_retail_20221027.pkl"))
...
dag = DAG(
    ...
    description="Run Subscription Cancellation forecasting model for germany retail",
    start_date=datetime(2023, 8, 1),
    schedule_interval=get_schedule("55 6 1 * *"),
    max_active_runs=1,
    catchup=False,
    tags=["Automation", "DS"]
)
...
cancellation_forecasting_germany_retail_1 = PythonOperator(
    dag=dag,
    task_id="cancellation_forecasting_germany_retail_1",
    python_callable=cancellation_forecasting_germany_retail_1,
    executor_config={
        "KubernetesExecutor": {
...
t0 >> cancellation_forecasting_germany_retail_1
