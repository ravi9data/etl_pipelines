...
import dags.cancellation_forecasting.germany_retail.constants as const
...
def create_dataframe(
    ...
    # Targets
    if use_targets:
        daily_targets, monthly_targets = query_targets(country=const.COUNTRY,
                                                       customer_type='Retail')
    else:
        monthly_targets = query_dynamic_targets(country=const.COUNTRY_ISO, customer_type='Retail')
    ...
    for feature_name in ['y_lag_1m', 'n_cohort_subscriptions_lag1m', 'n_cohort_subscriptions',
                         'cohort_asv']:
    ...
    logging.info("Project future acquisitions targets for the subcategories "
                 "that use an RF model that doesn't support null values")
    n_subc_projections = predict_ts_targets(cohort_data, const.RF_SUBCATEGORIES,
