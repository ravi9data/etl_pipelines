...
WITH customer AS (
 	...
 	c.id,
    c.direction,
    c.started_at::timestamp as started_at,
    c.answered_at::timestamp as answered_at,
    c.ended_at::timestamp as ended_at,
    c.duration,
    c.raw_digits,
    ...
    COALESCE(u.name, 'not available'::character varying) AS agent_name,
        ...
        END AS is_voicemail,
    c1.customer_id
   FROM s3_spectrum_rds_dwh_aircall.aircall_call c
     LEFT JOIN s3_spectrum_rds_dwh_aircall.aircall_user u ON c.user_id = u.id
 ...
 drop table if exists ods_external.aircall_subscription_payment;
