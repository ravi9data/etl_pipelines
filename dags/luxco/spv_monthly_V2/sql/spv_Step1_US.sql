...
DROP TABLE IF EXISTS ods_spv_historical.spv_used_asset_price_{{ params.tbl_suffix }}_us;
create table ods_spv_historical.spv_used_asset_price_{{ params.tbl_suffix }}_us as with d as
(
   /*#############################################
...
###############################################*/
   ...
   from
      (
         ...
         where
            week_date::date < '{{ params.first_day_of_month }}'::date
      )
   group by
      1,
      2
...
###################################################*/
   select DISTINCT
      ...
      mm.avg_mm_price
   from
      ods_spv_historical.union_sources_{{ params.tbl_suffix }}_us s		---*************
      left join
         d mm
...
(
   ...
   where
      ...
      purchased_date::date <= '{{ params.last_day_of_prev_month }}'
...
select
   '{{ params.first_day_of_month }}'::date - 1 as reporting_date,
   prep.*,
...
------STEP 3---------
drop table if exists ods_spv_historical.spv_used_asset_price_master_{{ params.tbl_suffix }}_us;
---*************
create table ods_spv_historical.spv_used_asset_price_master_{{ params.tbl_suffix }}_us as ---*************
WITH a AS
(
   SELECT
   ...
   FROM
      ods_spv_historical.spv_used_asset_price_{{ params.tbl_suffix }}_us u		---*************
   WHERE
      u.asset_condition <> 'Others'::text
      AND u.product_sku IS NOT NULL
      AND u.price IS NOT NULL
      and u.reporting_date = '{{ params.last_day_of_prev_month }}'::date 		---*************
   GROUP BY
...
SELECT
   ...
   CASE
      ...
      ELSE
         NULL::text
   END
) AS max_available_price_date, datediff(MONTH, b.max_available_reporting_month::date, '{{ params.last_day_of_prev_month }}'::Date) AS months_since_last_price, power(0.97, datediff(MONTH, b.max_available_reporting_month::date, '{{ params.last_day_of_prev_month }}'::Date)) * avg(
   CASE
      WHEN
         date_trunc('month', a.reporting_month::date) = date_trunc('month', b.max_available_reporting_month::date)
      THEN
...
------STEP 4-----------------
DROP TABLE IF EXISTS ods_spv_historical.price_per_condition_{{ params.tbl_suffix }}_us;
---*************
create table ods_spv_historical.price_per_condition_{{ params.tbl_suffix }}_us AS    ---*************
WITH a AS
(
   SELECT DISTINCT
      b.product_sku,
   ...
   FROM
      ods_spv_historical.spv_used_asset_price_master_{{ params.tbl_suffix }}_us b		---*************
   where
      b.reporting_date = '{{ params.last_day_of_prev_month }}'::date
   GROUP BY
      b.product_sku
)
SELECT
...
-----STEP 5--------
drop table if exists ods_spv_historical.asset_market_value_{{ params.tbl_suffix }}_us;
---*************
create table ods_spv_historical.asset_market_value_{{ params.tbl_suffix }}_us as---*************
with d AS
(
   SELECT
      asset.asset_id,
...
#####################*/
         WHEN
            ...
            (
               1 - 0.03 * ((date_trunc('month', '{{ params.last_day_of_prev_month }}'::date)::date - ss.purchased_date::date) / 30)
            )
...
#####################*/
         ...
         THEN
            ...
            (
               1 - 0.03 * ((date_trunc('month', '{{ params.last_day_of_prev_month }}'::date - 1)::date - ss.purchased_date::date) / 30)
            )
            * COALESCE(ss.initial_price, ss.amount_rrp, e.initial_price)))
         WHEN
            (
(1 - 0.03 * ((date_trunc('month', '{{ params.last_day_of_prev_month }}'::date)::date - ss.purchased_date) / 30)) * COALESCE(ss.initial_price, ss.amount_rrp, e.initial_price)
            )
            <= 0
         THEN
            0
...
------STEP 6--------
drop table if exists ods_production.spv_report_master_{{ params.tbl_suffix }}_us;
---*************
create table ods_production.spv_report_master_{{ params.tbl_suffix }}_us as---*************
SELECT distinct
   '{{ params.last_day_of_prev_month }}'::date as reporting_date,
   a.asset_id,
   a.warehouse,
   ...
   COALESCE(a.sold_date::date, '{{ params.first_day_of_month }}'::date) - a.purchased_date::date AS days_since_purchase,
   mv.valuation_method,
