DROP TABLE IF EXISTS dwh.crm_monthly_report;
CREATE TABLE dwh.crm_monthly_report AS 
...
, a AS (
	SELECT DISTINCT 
		...
		(CASE WHEN (DATEADD('day',-1,DATE_TRUNC('day',CURRENT_DATE))) = date THEN 1 ELSE 0 END) AS month_to_date,
		c.customer_id,
		customer_type,
		created_at,
	...
	LEFT JOIN countries_sessions s
		...
		AND row_sessions = 1
	WHERE (day_is_last_of_month = 1 OR date = DATEADD('day',-1,DATE_TRUNC('day',CURRENT_DATE)))
		AND date < current_date
)
,dc_churn_customers AS (
	SELECT 
		customer_id, 
		DATE_TRUNC('month',cancellation_date::date) AS cancellation_date,
		true AS is_dc
...
,prep AS (
	...
	LEFT JOIN dc_churn_customers dc
		...
		AND a.month_date=dc.cancellation_date
)
,agg AS (
	SELECT DISTINCT 
		date_trunc('month',date)::date as bom,
		date::date as eom,
		month_to_date,
		customer_type,
		country,
...
,registered AS (
	SELECT  
		date_trunc('month',created_at)::date as fact_date,
		customer_type,
		CASE WHEN c.signup_country <> 'never_add_to_cart' THEN c.signup_country
			ELSE s.country END AS country,
...
,switch AS (
	SELECT
	 	date_trunc('month',Date)::Date as fact_date, 
	 	customer_type,
...
GRANT SELECT ON dwh.crm_monthly_report TO tableau;
