drop table if exists ods_production.variant;
create table ods_production.variant as
with v as (
	select  
		v.id as variant_id,
	...
	left join stg_salesforce.product2 a on a.sku_variant__c=v.sku
  	left join stg_api_production.variant_stores s on s.variant_id=v.id 
	inner join stg_api_production.spree_products p on v.product_id=p.id 
	where true 
		--and  store_id='1'
	...
	left join stg_salesforce.product2 p on p.sku_variant__c=a.f_product_sku_variant__c
	left join v on a.f_product_sku_variant__c=v.variant_sku
	left join stg_api_production.spree_variants v2 on v2.sku=a.f_product_sku_variant__c
	...
	select 
		...
		b.availability_state,
  		...
  		b.article_number
	from b
		inner join stg_api_production.spree_variants v2 on v2.id=b.variant_id
...
GRANT SELECT ON ods_production.variant TO redash_commercial, redash_pricing;
GRANT SELECT ON ods_production.variant TO tableau;
GRANT SELECT ON ods_production.variant TO GROUP mckinsey;
