...
create temp table product_final as
with category as (
select
...
left join stg_fraud_and_credit_risk.subcategory_brand_risk_labels rl on (lower(c.subcategory_name) = rl.subcategory  and lower(p.brand) = rl.brand)
where p.id != 9251;

delete from ods_production.product where 1=1;

insert into ods_production.product 
select * from product_final;

grant select on  ods_production.product to group risk_users;
GRANT SELECT ON ods_production.product TO GROUP mckinsey;
