drop table if exists ods_production.store;
create table ods_production.store as
 WITH
 ...
 select store_id,
	...
	left join stg_flowing_mosel.auth_user_groups u ---user id and group id
 ...
 a AS (
     ...
     greatest(store.created_at, store.updated_at) as updated_date
           FROM stg_api_production.spree_stores store
   		   left join stg_api_production.partners p on store.partner_id=p.id
          left join stg_api_production.spree_countries c on c.id=store.country_id
   	   		left join public.partner_store_codes code on code.backofficestoreid = store.id
   		       )
 SELECT
    a.id,
 	a.store_number,
 ...
 --	s.bundesland as state,
   ...
   --left join public.dim_states s on a.zipcode = s.plz
   ;

GRANT SELECT ON ods_production.store TO mahmoudmando, redash_pricing, GROUP pricing;
GRANT SELECT ON ods_production.store TO tableau;
