CREATE OR REPLACE VIEW dm_risk.v_orders_in_manual_review_report_hourly AS
WITH dates AS (
	SELECT 
...
WHERE 	
	CASE WHEN outcome_namespace IN ('APPROVED', 'DECLINED') AND rn2 = 1 THEN 1
		 WHEN outcome_namespace IN ('APPROVED', 'DECLINED') AND fact_date::date = published_at::date THEN 1
