DROP TABLE IF EXISTS segment.page_views_web;
CREATE TABLE segment.page_views_web
    DISTKEY(session_id) 
...
SELECT
    ...
    DENSE_RANK() OVER (PARTITION BY a.anonymous_id order by a.session_id) AS session_index,
    a.event_id AS page_view_id,
    ROW_NUMBER() OVER (PARTITION BY a.anonymous_id ORDER BY a.event_time) AS page_view_index,
    ROW_NUMBER() OVER (PARTITION BY a.session_id ORDER BY a.event_time) AS page_view_in_session_index,
    ...
    dm.device_platform AS os_family, --we don't have actuall os_name here, but this is what we receive from segment and it is enough to do mapping that we need
    NULL AS os_timezone,
...
WHERE a.user_agent NOT ILIKE '%datadog%';

GRANT SELECT ON segment.page_views_web TO tableau;
GRANT SELECT ON segment.page_views_web TO hams;
