...
CREATE TEMP TABLE tmp_segment_session_marketing_mapping_web AS
...
session_unique AS (
    ...
    FROM segment.all_events a
        ...
        INNER JOIN last_3_days_sessions c USING (session_id)
    WHERE platform = 'web'
    ),
...
SELECT
    ...
    CASE
        ...
        WHEN marketing_medium = 'cpc'
            ...
            THEN 
            CASE WHEN marketing_campaign ILIKE '%brand%'
...
DELETE FROM segment.session_marketing_mapping_web
    USING tmp_segment_session_marketing_mapping_web b
WHERE session_marketing_mapping_web.session_id = b.session_id;

INSERT INTO segment.session_marketing_mapping_web
SELECT *
FROM tmp_segment_session_marketing_mapping_web;

END TRANSACTION;

DROP TABLE IF EXISTS tmp_segment_session_marketing_mapping_web;
