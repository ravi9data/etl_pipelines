--full load of conversion funnel
...
WITH first_page_in_session AS (
    ...
    WHERE page_view_start::DATE BETWEEN DATEADD('month', -13, current_date) AND DATEADD('day', -1, current_date)
)
...
WHERE pv.page_view_date::date BETWEEN DATEADD('month', -13, current_date) AND DATEADD('day', -1, current_date)
GROUP BY 1,2,3,4,5,6,7,8
;
...
WHERE ai."timestamp"::date BETWEEN DATEADD('month', -13, current_date) AND DATEADD('day', -1, current_date)
GROUP BY 1,2,3,4,5,6,7,8
;
...
FROM tmp_conversion_funnel_daily_report_traffic o
