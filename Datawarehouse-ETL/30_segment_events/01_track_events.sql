...
DROP TABLE IF EXISTS segment.track_events;
CREATE TABLE segment.track_events 
DISTKEY(session_id)
...
SELECT
    ...
    c."traits"."marketing_channel"::VARCHAR AS marketing_channel_traits,
    c."traits"."marketing_consent"::VARCHAR AS marketing_consent_traits,
    ...
    "properties",
    COALESCE(p."order_id",p."order_number",p."orderID",p."props"."order_id")::VARCHAR AS order_id,
    COALESCE(p."store_id", p."props"."store_id")::INT AS store_id,
    COALESCE(p."store_code",p."store")::VARCHAR AS store_code,
    "traits",
    "sent_at",
    "received_at",
    "loaded_at"
...
SELECT
    ...
    "page_url",
    "user_agent",
    "marketing_channel_traits",
    "marketing_consent_traits",
...
UPDATE segment.track_events SET marketing_source = 'ebay_kleinanzeigen' WHERE marketing_source ILIKE '%ebay_kleinanzeigen%';
UPDATE segment.track_events SET traits_source = 'ebay_kleinanzeigen' WHERE traits_source ILIKE '%ebay_kleinanzeigen%';

GRANT SELECT ON segment.track_events TO redash_pricing;
GRANT SELECT ON segment.track_events TO tableau;
GRANT SELECT ON segment.track_events TO hams;
