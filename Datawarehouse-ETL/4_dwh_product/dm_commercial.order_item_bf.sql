--LIVE_REPORTING.ORDER_ITEM



DROP TABLE IF EXISTS dm_commercial.order_item_bf;
CREATE TABLE dm_commercial.order_item_bf AS
--new_infra    
WITH numbers AS (
  	SELECT
		*
...
, order_items_all AS (
	...
	FROM dm_commercial.order_bf o --filtering the same orders we have IN live_reporting.ORDER
...
SELECT DISTINCT
	...
	p.product_name,
	oi.current_local_timestamp
FROM dm_commercial.order_bf oi --filtering the same orders we have IN live_reporting.ORDER
...
-- Adding condition to remove carts that do not exist in sf (provided that order exists in sf
	...
	GRANT SELECT ON dm_commercial.order_item_bf TO tableau;
	GRANT SELECT ON dm_commercial.order_item_bf TO GROUP BI;
