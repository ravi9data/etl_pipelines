...
, customer_data AS (
	SELECT 
		datum AS c_date, 
		customer_id,
		CASE WHEN c.card_created_date IS NOT NULL AND date_trunc('month',card_created_date)::date<=datum::date THEN 1 ELSE 0 END AS card_created
	FROM dates dd 
...
,deposit_data AS (
	SELECT 
		datum AS d_date, 
		customer_id,
		CASE WHEN d.deposit_date IS NOT NULL AND date_trunc('month',deposit_date)::date<=datum::date THEN 1 ELSE 0 END AS money_deposited
	FROM dates dd 
...
,closed_accounts AS (
	SELECT 
		datum AS ca_date, 
		customer_id,
		CASE WHEN legal_closure_date IS NOT NULL AND date_trunc('month',legal_closure_date)::date<=datum::date THEN 1 ELSE 0 END AS account_closed
	FROM dates dd 
...
,payments as (
	SELECT 
		DISTINCT customer_id,
		date_trunc('month',transaction_date) AS t_date 
...
CASE 
	...
	END 
AS MAU_status,
LAG(mau_status)OVER(PARTITION BY customer_id ORDER BY month_) AS Last_Mau_status
FROM FINAL )
SELECT *,
