--customer_churn_report_by_countries
drop view if exists dm_finance.v_customer_churn_report_by_countries;
create view dm_finance.v_customer_churn_report_by_countries as
with dates as (
select distinct 
 datum as fact_date, 
...
select 
 ...
 sum(s.subscription_value) as active_subscription_value
  from dates d
left join (select s.subscription_id,s.store_label, s.store_commercial, s.customer_id,s.country_name,s.customer_type, s.subscription_value, s.start_date, s.cancellation_date, c.cancellation_reason_churn,s.rental_period
...
select 
 ...
 sum(s.subscription_value) as total_subscription_value
  from dates d
left join (select s.subscription_id,s.store_label, s.store_commercial,s.country_name, s.customer_id,s.customer_type, s.subscription_value, s.start_date, s.cancellation_date, c.cancellation_reason_churn
 from master.subscription s 
 left join ods_production.subscription_cancellation_reason c 
 on s.subscription_id=c.subscription_id
 where rental_period > 1 ---EXCLUDE all one month subscriptions
...
(
select customer_id,country_name, DATE_TRUNC('month',cancellation_date::date) as cancellation_date,
true as is_dc
...
from prep p 
   left join dc_churn_customers dc on dc.cancellation_date = month_bom and dc.customer_id = p.customer_id and dc.country_name = p.country_name
)
	, agg as (
select distinct 
 month_bom as bom,
