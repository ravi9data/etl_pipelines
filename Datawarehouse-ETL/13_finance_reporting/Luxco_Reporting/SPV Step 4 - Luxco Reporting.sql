...
WHERE
   TRUE
   AND DATE_TRUNC('MONTH', s.date) = '2023-05-01'
...
WHERE
   TRUE
   AND DATE_TRUNC('MONTH', s.date)::DATE = '2023-05-01'
   AND a.allocation_status_original NOT IN
   (
      'CANCELLED'
   )
...
WHERE
   date = '2023-05-31' ;
-------------------------------------------------------------------------------------------------------------------------------------
DROP TABLE IF EXISTS ods_spv_historical.spv_report_master_202305;
CREATE TABLE ods_spv_historical.spv_report_master_202305 AS
SELECT
   *
FROM
   ods_production.spv_report_master_202305 ;
-------------------------------------------------------------------------------------------------------------------------------------
...
WHERE
   reporting_date = '2023-05-31'
;
-------------------------------------LUXCO REPORTING----------------------------------------------------------------------------------------------
-------------------------------------LUXCO REPORTING----------------------------------------------------------------------------------------------
DROP TABLE IF EXISTS ods_spv_historical.luxco_reporting_202305;
CREATE TABLE ods_spv_historical.luxco_reporting_202305 AS
(
...
,
   ...
   (
      ...
      WHERE
         reporting_date = DATE('2023-05-01') - 1
   )
,
   allocation_pre AS
   (
 ...
 , '[^[:alnum:][:blank:][:punct:]]', ''), ' ', ' ')
      ...
      CASE
         ...
         ELSE
            FALSE
      END
      AS is_risky_customer, ROUND((NULLIF(h.market_price_at_purchase_date, 0) - COALESCE(h.initial_price, 0)) / NULLIF(h.market_price_at_purchase_date, 0), 2) AS asset_purchase_discount_percentage, h.market_price_at_purchase_date,
      CASE
         WHEN
            s.subscription_id IS NOT NULL
            AND s.status <> 'CANCELLED'
      ...
      * original_valuation AS final_valuation_before_impairment_m_n_g,
      case when spv.asset_status_original like 'WRITTEN OFF%' then spv.final_price_without_written_off else original_valuation end as original_valuation_without_written_off
   FROM
      ods_spv_historical.spv_report_master_202305 spv
      LEFT JOIN
   ...
   WHERE
      spv.reporting_date = '2023-05-31' ) ;
-----------------------------------------LUCXO REPORTING END--------------------------------------------------------------------------------------------
-----------------------------------------LUCXO REPORTING END--------------------------------------------------------------------------------------------
GRANT SELECT ON ods_spv_historical.luxco_reporting_202305 to elene_tsintsadze;
GRANT SELECT ON ods_spv_historical.luxco_reporting_202305 to paolo_giambona;
GRANT SELECT ON ods_spv_historical.luxco_reporting_202305 TO accounting_redash;
GRANT SELECT ON ods_spv_historical.supplier TO tableau;
