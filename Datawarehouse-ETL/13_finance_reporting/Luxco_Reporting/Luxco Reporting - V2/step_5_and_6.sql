-----STEP 5--------			   
drop table if exists ods_spv_historical.asset_market_value_202407_eu;
---*************
create table ods_spv_historical.asset_market_value_202407_eu as---*************
with d AS
(
   SELECT
      asset.asset_id,
...
#####################*/
         ...
         WHEN
            (
(1 - 0.03 * ((date_trunc('month', '2024-07-31'::date)::date - ss.purchased_date) / 30)) * COALESCE(f.gross_purchase_price_corrected, ss.initial_price, ss.amount_rrp, e.initial_price)
            )
            <= 0
         THEN
            0
...
------STEP 6--------
drop table if exists ods_production.spv_report_master_202407_eu;
---*************
create table ods_production.spv_report_master_202407_eu as---*************
SELECT distinct
   '2024-07-31'::date as reporting_date,
   a.asset_id,
   a.warehouse,
