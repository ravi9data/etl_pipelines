...
DROP TABLE IF EXISTS ods_spv_historical.spv_used_asset_price_202407_us;
create table ods_spv_historical.spv_used_asset_price_202407_us as with d as
(
   /*#############################################
...
###################################################*/
   select DISTINCT
      ...
      mm.avg_mm_price
   from
      ods_spv_historical.union_sources_202407_us s		---*************
      left join
         d mm
...
------STEP 3---------
drop table if exists ods_spv_historical.spv_used_asset_price_master_202407_us;
---*************
create table ods_spv_historical.spv_used_asset_price_master_202407_us as ---*************
WITH a AS
(
   SELECT
   ...
   FROM
      ods_spv_historical.spv_used_asset_price_202407_us u		---*************
   WHERE
...
------STEP 4-----------------
DROP TABLE IF EXISTS ods_spv_historical.price_per_condition_202407_us;
---*************
create table ods_spv_historical.price_per_condition_202407_us AS    ---*************
WITH a AS
(
   SELECT DISTINCT
      b.product_sku,
   ...
   FROM
      ods_spv_historical.spv_used_asset_price_master_202407_us b		---*************
   where
...
-----STEP 5--------
drop table if exists ods_spv_historical.asset_market_value_202407_us;
---*************
create table ods_spv_historical.asset_market_value_202407_us as---*************
with d AS
(
   SELECT
      asset.asset_id,
...
#####################*/
         WHEN
            ...
            (
               1 - 0.03 * ((date_trunc('month', '2024-07-31'::date)::date - ss.purchased_date::date) / 30)
            )
...
#####################*/
         ...
         THEN
            ...
            (
               1 - 0.03 * ((date_trunc('month', '2024-07-31'::date - 1)::date - ss.purchased_date::date) / 30)
            )
            * COALESCE(ss.initial_price, ss.amount_rrp, e.initial_price)))
         WHEN
            (
(1 - 0.03 * ((date_trunc('month', '2024-07-31'::date)::date - ss.purchased_date) / 30)) * COALESCE(ss.initial_price, ss.amount_rrp, e.initial_price)
            )
            <= 0
         THEN
            0
...
(ss.asset_condition_spv = 'AGAN'
      ...
      AS valuation_2,
      (
         1 - 0.03 * ((date_trunc('month', '2024-07-31'::date)::date - ss.purchased_date) / 30)
      )
...
------STEP 6--------
drop table if exists ods_production.spv_report_master_202407_us;
---*************
create table ods_production.spv_report_master_202407_us as---*************
SELECT distinct
   '2024-07-31'::date as reporting_date,
   a.asset_id,
   a.warehouse,
   ...
   COALESCE(a.sold_date::date, '2024-08-01'::date) - a.purchased_date::date AS days_since_purchase,
   mv.valuation_method,
