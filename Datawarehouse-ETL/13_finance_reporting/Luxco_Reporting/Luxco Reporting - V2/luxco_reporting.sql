...
WHERE
   TRUE
   AND DATE_TRUNC('MONTH', s.date) = '2024-07-01'
...
WHERE
   TRUE
   AND DATE_TRUNC('MONTH', s.date)::DATE = '2024-07-01'
   AND a.allocation_status_original NOT IN
   (
      'CANCELLED'
   )
...
WHERE
   date = '2024-07-31' ;
-------------------------------------------------------------------------------------------------------------------------------------
DROP TABLE IF EXISTS ods_spv_historical.spv_report_master_202407;
CREATE TABLE ods_spv_historical.spv_report_master_202407 AS
SELECT
   *
FROM
...
-------------------------------------LUXCO REPORTING----------------------------------------------------------------------------------------------
-------------------------------------LUXCO REPORTING----------------------------------------------------------------------------------------------
DROP TABLE IF EXISTS ods_spv_historical.luxco_reporting_202407;
CREATE TABLE ods_spv_historical.luxco_reporting_202407 AS
(
...
,
   ...
   (
      ...
      WHERE
         reporting_date = DATE('2024-07-01') - 1
   )
,
   allocation_pre AS
   (
...
(original_valuation / prev.previous_original_valuation) - 1
 ...
 , '[^[:alnum:][:blank:][:punct:]]', ''), ' ', ' ')
	...
	THEN 
     ...
     FROM
      ods_spv_historical.spv_report_master_202407 spv
      LEFT JOIN
...
GRANT
SELECT
   ON ods_spv_historical.luxco_reporting_202407 to elene_tsintsadze;
   
GRANT
SELECT
   ON ods_spv_historical.luxco_reporting_202407 TO accounting_redash;
   
GRANT
SELECT
   ON ods_spv_historical.luxco_reporting_202407 TO fabio_zaim;