...
DROP TABLE IF EXISTS ods_spv_historical_test.spv_used_asset_price_1;
create table ods_spv_historical_test.spv_used_asset_price_1 as with d as
(
   /*#############################################
...
###################################################*/
   select DISTINCT
      ...
      mm.avg_mm_price
   from
      ods_spv_historical_test.union_sources_1 s		---*************
      left join
         d mm
...
order by
  ...
  ------------------------------------------------
  ------------------------------------------------
 drop table if exists ods_spv_historical_test.spv_used_asset_price_master_1;
---*************
create table ods_spv_historical_test.spv_used_asset_price_master_1 as ---*************
WITH a AS
(
   SELECT
   ...
   FROM
      ods_spv_historical_test.spv_used_asset_price_1 u		---*************
   WHERE
...
GROUP BY
  ...
  ------------------------------------------------
  ------------------------------------------------
  DROP TABLE IF EXISTS ods_spv_historical_test.price_per_condition_1;
---*************
create table ods_spv_historical_test.price_per_condition_1 AS---*************
WITH a AS
(
   SELECT DISTINCT
      b.product_sku,
   ...
   FROM
      ods_spv_historical_test.spv_used_asset_price_master_1 b		---*************
   where
