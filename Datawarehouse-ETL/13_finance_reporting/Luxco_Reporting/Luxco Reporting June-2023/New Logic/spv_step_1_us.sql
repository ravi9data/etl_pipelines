...
DROP TABLE IF EXISTS ods_spv_historical.spv_used_asset_price_202306_us_new;
create table ods_spv_historical.spv_used_asset_price_202306_us_new as with d as
(
   /*#############################################
...
###################################################*/
   select DISTINCT
      ...
      mm.avg_mm_price
   from
      ods_spv_historical.union_sources_202306_us s		---*************
      left join
         d mm
...
------STEP 3---------
drop table if exists ods_spv_historical.spv_used_asset_price_master_202306_us_new;
---*************
create table ods_spv_historical.spv_used_asset_price_master_202306_us_new as ---*************
WITH a AS
(
   SELECT
   ...
   FROM
      ods_spv_historical.spv_used_asset_price_202306_us_new u		---*************
   WHERE
...
------STEP 4-----------------
DROP TABLE IF EXISTS ods_spv_historical.price_per_condition_202306_us_new;
---*************
create table ods_spv_historical.price_per_condition_202306_us_new AS    ---*************
WITH a AS
(
   SELECT DISTINCT
      b.product_sku,
   ...
   FROM
      ods_spv_historical.spv_used_asset_price_master_202306_us_new b		---*************
   where
...
-----STEP 5--------
drop table if exists ods_spv_historical.asset_market_value_202306_us_new;
---*************
create table ods_spv_historical.asset_market_value_202306_us_new as---*************
with d AS
(
   SELECT
      asset.asset_id,
...
#####################*/
         ...
         WHEN
            (
(1 - 0.03 * ((date_trunc('month', '2023-06-01'::date - 1)::date - ss.purchased_date) / 30)) * COALESCE(ss.initial_price, ss.amount_rrp, e.initial_price)
            )
            <= 0
         THEN
            0
...
------STEP 6--------
drop table if exists ods_spv_historical.spv_report_master_202306_us_new;
---*************
create table ods_spv_historical.spv_report_master_202306_us_new as---*************
SELECT distinct
   '2023-06-30'::date as reporting_date,
   a.asset_id,
   a.warehouse,
...
;
GRANT SELECT ON ods_spv_historical.price_per_condition_202306_us_new TO tableau;
