...
DROP TABLE IF EXISTS ods_spv_historical.spv_used_asset_price_202306_eu;
create table ods_spv_historical.spv_used_asset_price_202306_eu as with d as
(
   /*#############################################
...
###############################################*/
   ...
   from
      (
         ...
         where
            week_date::date < '2023-07-01'::date
      )
   group by
      1,
      2
...
###################################################*/
   select DISTINCT
      ...
      mm.avg_mm_price
   from
      ods_spv_historical.union_sources_202306_eu s		---*************
      left join
         d mm
...
(
   ...
   WHERE
     ...
     purchased_date::date <= '2023-06-30'
   group by
      1,
      2
)
...
select
   '2023-07-01'::date - 1 as reporting_date,
   prep.*,
...
order by
   3 desc ;
------STEP 3---------
drop table if exists ods_spv_historical.spv_used_asset_price_master_202306_eu;
---*************
create table ods_spv_historical.spv_used_asset_price_master_202306_eu as ---*************
WITH a AS
(
   SELECT
   ...
   FROM
      ods_spv_historical.spv_used_asset_price_202306_eu u		---*************
   WHERE
      u.asset_condition <> 'Others'::text
      AND u.product_sku IS NOT NULL
      AND u.price IS NOT NULL
      and u.reporting_date = '2023-06-30'::date 		---*************
   GROUP BY
...
SELECT
   ...
   CASE
      ...
      ELSE
         NULL::text
   END
) AS max_available_price_date, datediff(MONTH, b.max_available_reporting_month::date, '2023-06-30'::Date) AS months_since_last_price, power(0.97, datediff(MONTH, b.max_available_reporting_month::date, '2023-06-30'::Date)) * avg(
   CASE
      WHEN
         date_trunc('month', a.reporting_month::date) = date_trunc('month', b.max_available_reporting_month::date)
      THEN
...
--------STEP 4-----------------
DROP TABLE IF EXISTS ods_spv_historical.price_per_condition_202306_eu;
---*************
create table ods_spv_historical.price_per_condition_202306_eu AS---*************
WITH a AS
(
   SELECT DISTINCT
      b.product_sku,
   ...
   FROM
      ods_spv_historical.spv_used_asset_price_master_202306_eu b		---*************
   where
      b.reporting_date = '2023-06-30'::date
   GROUP BY
      b.product_sku
)
SELECT
   '2023-06-30'::date as reporting_date,
   a.product_sku,
   a.neu_price,
...
FROM
   a;
-----STEP 5--------
drop table if exists ods_spv_historical.asset_market_value_202306_eu;
---*************
create table ods_spv_historical.asset_market_value_202306_eu as---*************
with d AS
(
   SELECT
      asset.asset_id,
...
#####################*/
         WHEN
            ...
            (
               1 - 0.03 * ((date_trunc('month', '2023-06-01'::date - 1)::date - ss.purchased_date::date) / 30)
            )
...
#####################*/
         ...
         WHEN
            (
(1 - 0.03 * ((date_trunc('month', '2023-06-01'::date - 1)::date - ss.purchased_date) / 30)) * COALESCE(f.gross_purchase_price_corrected ,ss.initial_price, ss.amount_rrp, e.initial_price)
            )
            <= 0
         THEN
            0
...
(ss.asset_condition_spv = 'AGAN'
      ...
      AS valuation_2,
      (
         1 - 0.03 * ((date_trunc('month', '2023-06-30'::date)::date - ss.purchased_date) / 30)
      )
      ...
      LEFT JOIN
         (
            select
               *
            from
               ods_spv_historical.price_per_condition_202306_eu
            where
               reporting_date::date =
               (
                  '2023-06-30'::date::date
               )
         )
         c
...
FROM
   f;
------STEP 6--------
drop table if exists ods_production.spv_report_master_202306_eu;
---*************
create table ods_production.spv_report_master_202306_eu as---*************
SELECT distinct
   '2023-06-30'::date as reporting_date,
   a.asset_id,
   a.warehouse,
   ...
   COALESCE(a.sold_date::date, '2023-07-01'::date) - a.purchased_date::date AS days_since_purchase,
   mv.valuation_method,
   ...
   LEFT JOIN
      (
         SELECT DISTINCT
            ...
            currency
         FROM
            ods_spv_historical.union_sources_202306_eu
      )
      use
   ...
   LEFT JOIN
      (
         select
            *
         from
            ods_spv_historical.asset_market_value_202306_eu
         where
            reporting_date =
            (
               '2023-06-30'::date
