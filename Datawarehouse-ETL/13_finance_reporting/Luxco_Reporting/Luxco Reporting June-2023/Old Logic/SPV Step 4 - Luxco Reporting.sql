...
WHERE
   TRUE
   AND DATE_TRUNC('MONTH', s.date) = '2023-06-01'
...
WHERE
   TRUE
   AND DATE_TRUNC('MONTH', s.date)::DATE = '2023-06-01'
   AND a.allocation_status_original NOT IN
   (
      'CANCELLED'
   )
...
WHERE
   date = '2023-06-30' ;
-------------------------------------------------------------------------------------------------------------------------------------
DROP TABLE IF EXISTS ods_spv_historical.spv_report_master_202306;
CREATE TABLE ods_spv_historical.spv_report_master_202306 AS
SELECT
   *
FROM
   ods_production.spv_report_master_202306 ;
-------------------------------------------------------------------------------------------------------------------------------------
...
-------------------------------------LUXCO REPORTING----------------------------------------------------------------------------------------------
-------------------------------------LUXCO REPORTING----------------------------------------------------------------------------------------------
DROP TABLE IF EXISTS ods_spv_historical.luxco_reporting_202306;
CREATE TABLE ods_spv_historical.luxco_reporting_202306 AS
(
...
,
   ...
   (
      ...
      WHERE
         reporting_date = DATE('2023-06-01') - 1
   )
,
   allocation_pre AS
   (
...
,
   ...
   SELECT DISTINCT
      ...
      spv.serial_number,
      acat.brand,
      REPLACE(REGEXP_REPLACE(TRANSLATE(spv.asset_name, 'uoe?oaauoaineaco�e', 'uoesoaauoaine acoe')
   ...
   FROM
      ...
      CASE
         WHEN
            ...
            AND s.status <> 'CANCELLED'
         THEN
            REPLACE(REGEXP_REPLACE(TRANSLATE(COALESCE(cust.shipping_city::TEXT, h.city::TEXT), 'uoe?oaauoaineaco�e', 'uoesoaauoaine acoe')
 , '[^[:alnum:][:blank:][:punct:]]', ''), ' ', ' ')
      END
      AS city,
      CASE
   ...
   FROM
      ods_spv_historical.spv_report_master_202306 spv
      LEFT JOIN
   ...
   WHERE
      spv.reporting_date = '2023-06-30' ) ;
-----------------------------------------LUCXO REPORTING END--------------------------------------------------------------------------------------------
-----------------------------------------LUCXO REPORTING END--------------------------------------------------------------------------------------------
GRANT
SELECT
   ON ods_spv_historical.luxco_reporting_202306 to elene_tsintsadze;
GRANT
SELECT
   ON ods_spv_historical.luxco_reporting_202306 to paolo_giambona;
GRANT
SELECT
   ON ods_spv_historical.luxco_reporting_202306 TO accounting_redash;