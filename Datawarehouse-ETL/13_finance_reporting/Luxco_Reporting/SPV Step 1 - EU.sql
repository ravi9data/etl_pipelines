...
DROP TABLE IF EXISTS ods_spv_historical.spv_used_asset_price_202305_eu;
create table ods_spv_historical.spv_used_asset_price_202305_eu as with d as
(
   /*#############################################
...
###################################################*/
   select DISTINCT
      ...
      mm.avg_mm_price
   from
      ods_spv_historical.union_sources_202305_eu s		---*************
      left join
         d mm
...
(
   SELECT distinct
      ...
      avg(a.initial_price) AS avg_pp_price
   FROM
...
order by
   3 desc ;
------STEP 3---------
drop table if exists ods_spv_historical.spv_used_asset_price_master_202305_eu;
---*************
create table ods_spv_historical.spv_used_asset_price_master_202305_eu as ---*************
WITH a AS
(
   SELECT
   ...
   FROM
      ods_spv_historical.spv_used_asset_price_202305_eu u		---*************
   WHERE
...
--------STEP 4-----------------
DROP TABLE IF EXISTS ods_spv_historical.price_per_condition_202305_eu;
---*************
create table ods_spv_historical.price_per_condition_202305_eu AS---*************
WITH a AS
(
   SELECT DISTINCT
      b.product_sku,
   ...
   FROM
      ods_spv_historical.spv_used_asset_price_master_202305_eu b		---*************
   where
...
FROM
   a;
-----STEP 5--------
drop table if exists ods_spv_historical.asset_market_value_202305_eu;
---*************
create table ods_spv_historical.asset_market_value_202305_eu as---*************
with d AS
(
   SELECT
      asset.asset_id,
...
FROM
   f;
------STEP 6--------
drop table if exists ods_production.spv_report_master_202305_eu;
---*************
create table ods_production.spv_report_master_202305_eu as---*************
SELECT distinct
   '2023-05-31'::date as reporting_date,
   a.asset_id,
   a.warehouse,
...
GRANT SELECT ON ods_spv_historical.price_per_condition_202305_eu TO tableau;
GRANT SELECT ON ods_spv_historical.price_per_condition_202306_eu TO tableau;
