...
DROP TABLE IF EXISTS ods_spv_historical.spv_used_asset_price_202307_eu;
create table ods_spv_historical.spv_used_asset_price_202307_eu as with d as
(
   /*#############################################
...
###############################################*/
   ...
   from
      (
         ...
         where
            week_date::date < '2023-08-01'::date
      )
   group by
      1,
      2
...
###################################################*/
   select DISTINCT
      ...
      mm.avg_mm_price
   from
      ods_spv_historical.union_sources_202307_eu s		---*************
      left join
         d mm
...
(
   ...
   WHERE
     ...
     purchased_date::date <= '2023-07-31'
   group by
      1,
      2
)
...
select
   '2023-08-01'::date - 1 as reporting_date,
   prep.*,
...
OR(prep.product_sku = 'GRB59P10386' and prep.asset_condition = 'Wie neu' and prep.extract_date = '2021-11-22' and item_id=13593)
...
----FORCE inclusion in December 2021
OR (prep.product_sku = 'GRB18P2641' and prep.asset_condition = 'Wie neu' and prep.extract_date = '2021-02-02' and item_id=5714207)
OR (prep.product_sku = 'GRB18P2359' and prep.asset_condition = 'Wie neu' and prep.extract_date = '2021-01-13' and item_id=5625892)
OR (prep.product_sku = 'GRB190P10903' and prep.asset_condition = 'Wie neu' and prep.extract_date = '2021-12-24' and item_id=69429)
OR (prep.product_sku = 'GRB41P10166' and prep.asset_condition = 'Wie neu' and prep.extract_date = '2021-12-08' and item_id=117243)*/
order by
   3 desc ;
------STEP 3---------
drop table if exists ods_spv_historical.spv_used_asset_price_master_202307_eu;
---*************
create table ods_spv_historical.spv_used_asset_price_master_202307_eu as ---*************
WITH a AS
(
   SELECT
   ...
   FROM
      ods_spv_historical.spv_used_asset_price_202307_eu u		---*************
   WHERE
      u.asset_condition <> 'Others'::text
      AND u.product_sku IS NOT NULL
      AND u.price IS NOT NULL
      and u.reporting_date = '2023-07-31'::date 		---*************
   GROUP BY
...
SELECT
   ...
   CASE
      ...
      ELSE
         NULL::text
   END
) AS max_available_price_date, datediff(MONTH, b.max_available_reporting_month::date, '2023-07-31'::Date) AS months_since_last_price, power(0.97, datediff(MONTH, b.max_available_reporting_month::date, '2023-07-31'::Date)) * avg(
   CASE
      WHEN
         date_trunc('month', a.reporting_month::date) = date_trunc('month', b.max_available_reporting_month::date)
      THEN
...
--------STEP 4-----------------
DROP TABLE IF EXISTS ods_spv_historical.price_per_condition_202307_eu;
---*************
create table ods_spv_historical.price_per_condition_202307_eu AS---*************
WITH a AS
(
   SELECT DISTINCT
      b.product_sku,
   ...
   FROM
      ods_spv_historical.spv_used_asset_price_master_202307_eu b		---*************
   where
      b.reporting_date = '2023-07-31'::date
   GROUP BY
      b.product_sku
)
SELECT
   '2023-07-31'::date as reporting_date,
   a.product_sku,
   a.neu_price,
...
FROM
   a;
-----STEP 5--------
drop table if exists ods_spv_historical.asset_market_value_202307_eu;
---*************
create table ods_spv_historical.asset_market_value_202307_eu as---*************
with d AS
(
   SELECT
      asset.asset_id,
...
#####################*/
         WHEN
            ...
            (
               1 - 0.03 * ((date_trunc('month', '2023-07-01'::date - 1)::date - ss.purchased_date::date) / 30)
            )
...
#####################*/
         ...
         WHEN
            (
(1 - 0.03 * ((date_trunc('month', '2023-07-01'::date - 1)::date - ss.purchased_date) / 30)) * COALESCE(f.gross_purchase_price_corrected ,ss.initial_price, ss.amount_rrp, e.initial_price)
            )
            <= 0
         THEN
            0
...
(ss.asset_condition_spv = 'AGAN'
      ...
      AS valuation_2,
      (
         1 - 0.03 * ((date_trunc('month', '2023-07-31'::date)::date - ss.purchased_date) / 30)
      )
      ...
      LEFT JOIN
         (
            select
               *
            from
               ods_spv_historical.price_per_condition_202307_eu
            where
               reporting_date::date =
               (
                  '2023-07-31'::date::date
               )
         )
         c
...
FROM
   f;
------STEP 6--------
drop table if exists ods_production.spv_report_master_202307_eu;
---*************
create table ods_production.spv_report_master_202307_eu as---*************
SELECT distinct
   '2023-07-31'::date as reporting_date,
   a.asset_id,
   a.warehouse,
   ...
   COALESCE(a.sold_date::date, '2023-08-01'::date) - a.purchased_date::date AS days_since_purchase,
   mv.valuation_method,
   ...
   LEFT JOIN
      (
         SELECT DISTINCT
            ...
            currency
         FROM
            ods_spv_historical.union_sources_202307_eu
      )
      use
   ...
   LEFT JOIN
      (
         select
            *
         from
            ods_spv_historical.asset_market_value_202307_eu
         where
            reporting_date =
            (
               '2023-07-31'::date
