...
DROP TABLE IF EXISTS ods_spv_historical.spv_used_asset_price_202307_us;
create table ods_spv_historical.spv_used_asset_price_202307_us as with d as
(
   /*#############################################
...
###################################################*/
   select DISTINCT
      ...
      mm.avg_mm_price
   from
      ods_spv_historical.union_sources_202307_us s		---*************
      left join
         d mm
...
order by
   3 desc ;
------STEP 3---------
drop table if exists ods_spv_historical.spv_used_asset_price_master_202307_us;
---*************
create table ods_spv_historical.spv_used_asset_price_master_202307_us as ---*************
WITH a AS
(
   SELECT
   ...
   FROM
      ods_spv_historical.spv_used_asset_price_202307_us u		---*************
   WHERE
...
------STEP 4-----------------
DROP TABLE IF EXISTS ods_spv_historical.price_per_condition_202307_us;
---*************
create table ods_spv_historical.price_per_condition_202307_us AS---*************
WITH a AS
(
   SELECT DISTINCT
      b.product_sku,
   ...
   FROM
      ods_spv_historical.spv_used_asset_price_master_202307_us b		---*************
   where
...
FROM
   a;
-----STEP 5--------
drop table if exists ods_spv_historical.asset_market_value_202307_us;
---*************
create table ods_spv_historical.asset_market_value_202307_us as---*************
with d AS
(
   SELECT
      asset.asset_id,
...
#####################*/
         ...
         WHEN
            (
(1 - 0.03 * ((date_trunc('month', '2023-07-01'::date - 1)::date - ss.purchased_date) / 30)) * COALESCE(ss.initial_price, ss.amount_rrp, e.initial_price)
            )
            <= 0
         THEN
            0
      ...
      LEFT JOIN
         (
            select
               *
            from
               ods_spv_historical.price_per_condition_202307_us
            where
               reporting_date::date =
               (
...
FROM
   f;
------STEP 6--------
drop table if exists ods_production.spv_report_master_202307_us;
---*************
create table ods_production.spv_report_master_202307_us as---*************
SELECT distinct
   '2023-07-31'::date as reporting_date,
   a.asset_id,
   a.warehouse,
   ...
   LEFT JOIN
      (
         SELECT DISTINCT
            ...
            currency
         FROM
            ods_spv_historical.union_sources_202307_us
      )
      use
   ...
   LEFT JOIN
      (
         select
            *
         from
            ods_spv_historical.asset_market_value_202307_us
         where
            reporting_date =
            (
               '2023-07-31'::date
