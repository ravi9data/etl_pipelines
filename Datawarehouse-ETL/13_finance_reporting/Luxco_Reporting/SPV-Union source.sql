...
(
   ...
   WHERE
      reporting_month::date < '2023-06-01'::date
   UNION ALL
   select distinct
      e.src,
      e.region,
...
drop table if exists ods_spv_historical.union_sources_202305_us;
drop table if exists ods_spv_historical.union_sources_202305_eu;
create table ods_spv_historical.union_sources_202305_eu as
select
   *
from
   tmp_union_sources
where
   region = 'GERMANY';
create table ods_spv_historical.union_sources_202305_us as
select
   *
from
   tmp_union_sources
where
   region = 'USA';
insert into
   ods_spv_historical.union_sources_202305_eu ( src, region, extract_date, reporting_month, item_id, product_sku, asset_condition, currency, price, price_in_euro)
   select
      'MR' as src,
      'GERMANY' as region,
   ...
   where
      ...
      and mr.luxco_month <= '2023-05-01';
insert into
   ods_spv_historical.union_sources_202305_us ( src, region, extract_date, reporting_month, item_id, product_sku, asset_condition, currency, price, price_in_euro)
   select
      'MR' as src,
      'USA'as region,
      luxco_month::date - 1 as extract_date,
...
drop table if exists ods_spv_historical.union_sources_202305_eu_updated;
create table ods_spv_historical.union_sources_202305_eu_updated as
with a as (
select *,
   case 
...
from ods_spv_historical.union_sources_202305_eu
)
select 
   a.product_sku,
   a.asset_condition,
...
drop table if exists ods_spv_historical.union_sources_202305_eu_updated_2;
create table ods_spv_historical.union_sources_202305_eu_updated_2 as
...
from ods_spv_historical.union_sources_202305_eu a 
left join ods_spv_historical.union_sources_202305_eu_updated b
on a.product_sku = b.product_sku
...
drop table if exists ods_spv_historical.union_sources_202305_eu;
create table ods_spv_historical.union_sources_202305_eu AS 
SELECT * FROM  ods_spv_historical.union_sources_202305_eu_updated_2
...
drop table if exists ods_spv_historical.union_sources_202305_us_updated;
create table ods_spv_historical.union_sources_202305_us_updated as
with a as (
select *,
   case 
      when asset_condition in ('Neu') then 'NEW'
...
from ods_spv_historical.union_sources_202305_us
)
select 
   a.product_sku,
   a.asset_condition,
...
drop table if exists ods_spv_historical.union_sources_202305_us_updated_2;
create table ods_spv_historical.union_sources_202305_us_updated_2 as
select a.*,b.asset_condition_2,price_LP,initial_price,
   case 
      when asset_condition_2 is not null and price < price_LP THEN 1
   else 0 end as outlier_removal 
from ods_spv_historical.union_sources_202305_us a 
left join ods_spv_historical.union_sources_202305_us_updated b
on a.product_sku = b.product_sku
and a.asset_condition = b.asset_condition 
;
...
drop table if exists ods_spv_historical.union_sources_202305_us;
create table ods_spv_historical.union_sources_202305_us AS 
SELECT * FROM  ods_spv_historical.union_sources_202305_us_updated_2
WHERE outlier_removal = 0;
