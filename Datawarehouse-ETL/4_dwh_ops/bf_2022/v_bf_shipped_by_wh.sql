create or replace view dm_operations.v_bf_shipped_by_wh AS
with shipped_assets as (
	select 
		date_trunc('hour',
					convert_timezone('CET', shipment_label_created_at)) as fact_date,
		region,
		count(distinct allocation_id) shipped_assets
	from ods_operations.allocation_shipment
	where shipment_label_created_at > '2022-11-07'
