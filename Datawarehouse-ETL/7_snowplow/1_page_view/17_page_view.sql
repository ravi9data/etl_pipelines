DROP TABLE IF EXISTS web.page_views_snowplow;
CREATE TABLE web.page_views_snowplow 
  DISTKEY(snowplow_user_id) AS (
SELECT
  a.root_id
 ...
 ,COALESCE(a.mkt_campaign, psc.mkt_campaign) AS marketing_campaign
...
FROM scratch.web_events AS a
  ...
  LEFT JOIN scratch.page_view_paid_search_campaigns psc 
    ON a.domain_sessionid = psc.session_id AND a.root_id = psc.root_id
WHERE TRUE
  AND page_view_date > '2019-06-01'
...
GRANT SELECT ON web.page_views_snowplow TO tableau;
