create temp table web_events_scroll_depth_dl (like scratch.web_events_scroll_depth);

  -- calculate how far a user has scrolled
insert into web_events_scroll_depth_dl
  WITH prep AS (
    SELECT
    ...
    WHERE ev.event_name IN ('page_view', 'page_ping','action')
      ...
      AND ev.etl_tstamp > DATEADD(WEEK, -1, CURRENT_DATE)
...
delete from scratch.web_events_scroll_depth
using web_events_scroll_depth_dl s
where scratch.web_events_scroll_depth.page_view_id = s.page_view_id;

insert into scratch.web_events_scroll_depth
select * from web_events_scroll_depth_dl;

end transaction;

drop table web_events_scroll_depth_dl;
