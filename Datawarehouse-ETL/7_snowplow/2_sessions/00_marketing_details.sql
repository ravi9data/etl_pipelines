DROP TABLE IF EXISTS web.session_marketing_mapping_snowplow;
CREATE TABLE web.session_marketing_mapping_snowplow AS 
WITH 
find_utms AS (
SELECT 
  a.session_id, 
...
WHERE medium_source_map != 'n/an/a'
  AND DATE(a.page_view_start) >= '2022-06-01'
...
WHERE pv.page_view_in_session_index = 1
   AND DATE(page_view_start) >= '2022-06-01'
)
...
SELECT DISTINCT 
  ...
  marketing_source,
  is_paid,
  COALESCE(NULLIF(b.marketing_campaign,'n/a'), psc.mkt_campaign, 'n/a') AS marketing_campaign,
  marketing_term,
  referer_url,
  is_internal,
  CASE WHEN is_display_criteo IS TRUE OR is_display_google IS TRUE OR is_display_other IS TRUE THEN TRUE ELSE FALSE END AS is_display,
  CASE WHEN is_paid_search_brand IS TRUE OR is_paid_search_generic IS TRUE THEN 
    CASE 
      WHEN COALESCE(NULLIF(b.marketing_campaign,'n/a'), psc.mkt_campaign) ILIKE '%trademark%' OR  
      COALESCE(NULLIF(b.marketing_campaign,'n/a'), psc.mkt_campaign) ILIKE '%brand%' THEN TRUE 
    ELSE FALSE END
  ELSE FALSE
  END AS is_paid_search_brand,
    CASE WHEN is_paid_search_brand IS TRUE OR is_paid_search_generic IS TRUE THEN 
    CASE 
      WHEN COALESCE(NULLIF(b.marketing_campaign,'n/a'), psc.mkt_campaign) ILIKE '%trademark%' OR 
      COALESCE(NULLIF(b.marketing_campaign,'n/a'), psc.mkt_campaign) ILIKE '%brand%' THEN FALSE 
    ELSE TRUE END
  ELSE FALSE
  END AS is_paid_search_non_brand,
  is_paid_social,
  FALSE AS is_shopping,
  is_refer_a_friend,
  is_influencers,
  is_affiliate,
  is_partnerships,
  ...
  is_organic_social,
  is_direct,
  CASE WHEN is_organic_search_brand IS TRUE OR is_organic_search_generic IS TRUE THEN TRUE ELSE FALSE END AS is_organic_search,
  is_crm,
  is_referral
FROM web.session_marketing_mapping_backup b
  LEFT JOIN scratch.page_view_paid_search_campaigns psc ON b.session_id = psc.session_id AND psc.rn = 1
WHERE DATE(page_view_start) < '2022-06-01' 
),

old_marketing_channel_mapping AS (
SELECT DISTINCT 
  session_id,
  page_view_start,
  CASE 
    WHEN marketing_channel = 'Paid Search' AND is_paid_search_brand = TRUE THEN 'Paid Search Brand'
    WHEN marketing_channel = 'Paid Search' AND is_paid_search_non_brand = TRUE THEN 'Paid Search Non Brand' 
    WHEN marketing_channel = 'Paid Search' AND is_paid_search_brand = FALSE AND is_paid_search_non_brand = FALSE THEN 
    CASE 
      WHEN marketing_campaign ILIKE '%trademark%' OR marketing_campaign ILIKE '%brand%' THEN 'Paid Search Brand'
    ELSE 'Paid Search Non Brand' END
  ELSE marketing_channel END AS marketing_channel,
  marketing_medium ,
  marketing_source,
  is_paid,
  marketing_campaign,
  ...
  is_crm,
  is_referral
FROM fix_channel_mapping
),

new_marketing_channel_mapping AS (
SELECT DISTINCT 
  m1.session_id,
  m1.page_view_start,
...
SELECT * FROM old_marketing_channel_mapping
UNION ALL
SELECT * FROM new_marketing_channel_mapping;

GRANT SELECT ON web.session_marketing_mapping_snowplow TO tableau;
