CREATE TEMP TABLE session_order_mapping_temp as 
WITH 
...
SELECT 
  m.order_id
FROM scratch.session_order_event_mapping m
...
SELECT 
  m.order_id
FROM scratch.session_order_url_mapping m
  INNER JOIN new_sessions s USING (session_id)    
...
SELECT 
  m.order_id
FROM scratch.session_order_sp_user_mapping m
  INNER JOIN new_sessions s USING (session_id)   
...
SELECT 
  m.order_id
FROM scratch.session_order_snowplow_user_mapping m
  INNER JOIN new_sessions s USING (session_id)   
WHERE order_id IS NOT NULL
),
...
SELECT 
 ...
 ,m.order_id
FROM scratch.session_order_event_mapping m
  INNER JOIN get_all_orders go USING (order_id)
...
SELECT 
  a.session_id
 ,a.order_id
 ,LISTAGG(DISTINCT a.src,' / ') WITHIN GROUP (ORDER BY a.src)  AS list_of_sources
FROM base a
GROUP BY 1,2
),
...
DELETE FROM web.session_order_mapping_snowplow 
USING session_order_mapping_temp b
WHERE session_order_mapping_snowplow.order_id = b.order_id 
;

INSERT INTO web.session_order_mapping_snowplow 
SELECT * 
FROM session_order_mapping_temp
;

END TRANSACTION;

DROP TABLE session_order_mapping_temp;