...
create table trans_dev.pricing_outlier_removal as 
with d as (
/*###############################
...
select 
	...
	round((s.price::double precision/
	        case 
	        	when coalesce(mm.avg_mm_price,c.avg_pp_price*1.1)=0 then null 
	        	else coalesce(mm.avg_mm_price,c.avg_pp_price*1.1) end
	       )*100,2) as coeff,
	case 
		when mm.avg_mm_price is not null then 100
		when round((s.price::double precision/
		            case 
		             	when coalesce(mm.avg_mm_price,c.avg_pp_price*1.1)=0 then null 
		             	else coalesce(mm.avg_mm_price,c.avg_pp_price*1.1) 
		             end
...
select 
	product_sku,
	asset_condition,
	median(median_coeff) as median_coeff 
from
	prep 
group by 
	1,2
...
select 
	...
	case 
		...
		when ((ref_price is  null) or ((coeff-c.median_coeff) between -20.00 and 20.00)) then 1 
