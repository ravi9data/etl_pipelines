drop table if exists dm_commercial.pricing_outlier_removal;
create table dm_commercial.pricing_outlier_removal as 
with d as (
     /*###############################
...
select
  ...
  avg(price::double precision) as avg_mm_price
...
where m.valid_from::date>DATEADD(day,-14, GETDATE()) --and  product_sku='GRB204P3935'
...
where  s.extract_date::date>DATEADD(day,-14, GETDATE()) 
--and s.product_sku='GRB204P3935' --and s.product_sku='GRB94P3748'--and mm.asset_condition='Wie neu' 
order by 1)
, e as ( 
      /*################################
...
SELECT distinct 
 ...
 where purchased_date::date <= '2019-12-31'::date
...
where true and s.extract_date::date>DATEADD(day,-14, GETDATE()) and asset_condition='Neu'
),
c as (
...
case 
		...
		when ((ref_price is  null) or ((coeff-c.median_coeff) between -10.00 and 10.00)) then 1 
